...
// The collection: find all of these and you earn the capybara_collector badge,
// which is the only way to unlock the Capybara sidekick in the Avatar Locker.
//
// Derived from status rather than hand-listed, so placing a new egg
// automatically raises the bar and retiring one automatically lowers it — a
// hardcoded roster would silently drift and either lock the sidekick forever or
// hand it out early. Only 'live' eggs count: a planned one is unfindable, and
// Slack's is not in the app at all.
...
// Has this learner found every findable egg? `found` is any iterable of ids.
export function isCollectionComplete(found) {
  const set = found instanceof Set ? found : new Set(found || []);
  return FINDABLE_EGG_IDS.length > 0 && FINDABLE_EGG_IDS.every((id) => set.has(id));
}

// Progress for the UI: how many of the collection this learner has seen.
export function collectionProgress(found) {
  const set = found instanceof Set ? found : new Set(found || []);
  const foundIds = FINDABLE_EGG_IDS.filter((id) => set.has(id));
  return {
    found: foundIds.length,
    total: FINDABLE_EGG_IDS.length,
    remaining: FINDABLE_EGG_IDS.filter((id) => !set.has(id)),
    complete: foundIds.length === FINDABLE_EGG_IDS.length && FINDABLE_EGG_IDS.length > 0,
  };
}
