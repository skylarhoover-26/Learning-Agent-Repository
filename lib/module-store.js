...
function ensureModule(state, num) {
  ...
  if (state.modules[num].quizAttempts === undefined) {
    state.modules[num].quizAttempts = state.modules[num].quizAnswer !== null ? 1 : 0;
  }
}
...
export function saveQuizAnswer(num, answer) {
  ...
  all.modules[num].quizAttempts = (all.modules[num].quizAttempts || 0) + 1;
  saveAllState(all);
}

export function resetQuizForRetry(num) {
  const all = getAllState();
  ensureModule(all, num);
  all.modules[num].quizAnswer = null;
  saveAllState(all);
}
