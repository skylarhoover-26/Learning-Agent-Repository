...
export const SCENARIOS = [
  {
    ...
    answers: [
      ...
      {
        text: "The renewal date's fine, but invoice amounts are sensitive financial data — strip those out first.",
        scores: { privacy: 0.5, prompting: 0.2 },
      },
      {
        text: "Even approved tools: paste only what the task needs; none of it shapes the pitch.",
  ...
  },
  {
    ...
    answers: [
      {
        text: "One prompt: 'Find the top 3 themes across all of these transcripts, and quote one example for each.'",
        scores: { prompting: 0.5, comms: 0.2 },
      },
      {
        text: 'Extract issues per transcript, cluster them, then re-read a few to verify.',
  ...
  },
  {
    ...
    answers: [
      ...
      {
        text: 'Click through to the source link, verify the actual document itself, then reply to them.',
        scores: { eval: 0.5, privacy: 0.2 },
      },
      {
        text: "Check what the policy said last month — AI only reflects today's docs.",
  ...
  },
  {
    ...
    answers: [
      ...
      {
        text: 'Have AI write the SQL behind those numbers, run it, then defend them.',
      ...
      {
        text: 'Manually average tenure on 10 churned and 10 retained rows; if it lines up, ship it.',
        scores: { data: 0.55, eval: 0.4 },
      },
    ],
  },
  {
    // Tool-agnostic on purpose: this measures the JUDGMENT of matching a model to
    // the task (fast/everyday vs. slower deep-reasoning), NOT knowledge of specific
    // model names — those change constantly and live in the model-lineup reference.
    // Kept curated (never AI-generated) so it can't be filled with stale model names.
    id: 'models',
    primary: 'models',
    ...
    answers: [
      {
        text: "Grab the fast model — it's quicker and its answers usually look fine.",
        scores: { models: 0.15 },
      },
      {
        text: 'Use the deep-reasoning model here; keep the fast one for routine tasks.',
        scores: { models: 1.0, eval: 0.3 },
      },
      {
        text: "Whatever's set as default — honestly I don't notice much difference.",
        scores: { models: 0.4 },
      },
      {
        text: 'Always pick the most powerful model so quality is never a question.',
        scores: { models: 0.3 },
      },
    ],
  },
...
// kept curated (compliance-sensitive); `models` is also curated-only (see above).
// The middle skills can be role-generated at runtime.
export const CALIBRATION_SKILL_ORDER = ['privacy', 'prompting', 'comms', 'eval', 'agents', 'data', 'models'];
