...
// React component.
//
// No 'use client' here on purpose: it's a data map and a pure lookup, and the
// directive would make it a client reference that server code can't call. The
// Slack lesson flow announces earned badges by name from the server, so this has
// to work on both sides. Client components can import a plain module freely.
export const BADGE_META = {
  ...
  // Awarded for finding every hidden capybara. This is the only badge whose
  // condition isn't in the progression ledger — see planCapybaraCollection.
  // It's also what unlocks the Capybara sidekick in lib/avatar-catalog.js.
  capybara_collector: { id: 'capybara_collector', name: 'Capybara Whisperer', emoji: '🦫' },
};
