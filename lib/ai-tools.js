...
// array of tool objects. Falls back to the default tool when empty.
export function normalizeTools(value) {
  ...
  return out.length ? out : [TOOL_BY_ID.get(DEFAULT_TOOL_ID)];
}

// Resolve the learner's set of tools (primary first). A per-session override
// (array or single) wins over the saved preference. Supports the legacy
// single `preferred_tool` field for profiles saved before multi-select.
export function resolveTools(profile, override) {
  if (override !== undefined && override !== null) return normalizeTools(override);
  const saved = profile?.preferred_tools ?? profile?.preferred_tool;
  return normalizeTools(saved);
}
...
export function serializeTools(tools) {
  return normalizeTools(tools).map((t) =>
    t.id === 'other' ? { id: 'other', label: t.label, url: t.url || null } : t.id
  );
}
...
// array; the first tool is treated as primary.
export function buildToolGuidance(toolsOrTool) {
  const tools = normalizeTools(toolsOrTool);
  const primary = tools[0];
  const theirKeys = new Set(tools.map(toolKey));
  const others = AI_TOOLS.filter((x) => !theirKeys.has(toolKey(x)))
