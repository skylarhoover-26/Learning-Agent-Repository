...
function buildCalibrationScenariosPrompt(learnerProfile) {
  ...
  return [
    ...
    '- setup (string): 1-2 SHORT sentences (~35 words max) describing a SPECIFIC, realistic moment from THIS PERSON\'S actual work — use their real tasks/role; name the kind of thing they\'d actually be doing. Keep it tight — no scene-setting fluff.',
    '- prompt (string): one short question (e.g. "How would you approach this?")',
    '- answers (array): EXACTLY 4 objects, each { "text": string, "scores": { <skillKey>: number } }. Each "text" is ONE short sentence (~18 words max) — scannable at a glance, never multi-sentence.',
    '',
    ...
    '- The 4 options are different ways THEY might handle it with AI — from a naive approach to how an expert in their role would do it. Keep each option parallel in length and phrasing.',
    ...
    '- VARY which option is best across the 6 scenarios (don\'t always put it last).',
    '',
    'Output rules:',
