...
export function buildLessonSystemPrompt(topic, learnerProfile, options = {}) {
  ...
  const tierGuidance = {
    beginner: [
      '- LEARNER LEVEL: BEGINNER — brand new to AI. Treat the following as HARD RULES, not suggestions:',
      '  • NEVER include code, SQL, terminal commands, config, or API/technical syntax — not even in examples.',
      '  • NEVER use unexplained jargon. If the topic itself is technical (e.g. RAG, embeddings, fine-tuning, APIs), do NOT teach the implementation. Reframe it entirely around what it means in plain English, when it helps them, and a simple real-world analogy.',
  ...
  return [
    ...
    tierBlock,
    !isDevTier
      ? '- Practice happens in the AI Chat built into this platform — never direct learners to external AI tools.'
      : null,
    display_name ? `- The learner's name is ${display_name}.` : null,
    department ? `- They work in ${department}${sub_team ? ` (${sub_team})` : ''}.` : null,
    tasksContext,
...
export function buildVideoScriptPrompt(topic, learnerProfile, options = {}) {
  ...
  return [
    ...
    '- TOOL NAMING (STRICT): Never name a specific AI chatbot like ChatGPT or Claude. Refer to "the AI Chat built into this platform" for practice, or "a large language model (LLM) like Gemini" in general. Never tell them to leave this app.',
    '',
    'SCRIPT RULES:',
