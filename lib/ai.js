...
const PLAN_SHAPE = {
  quick_tip: { objectives: '1', label: 'Quick Tip (60-second insight)', steps: '3' },
...
const PLAN_MAX_STEPS = { quick_tip: 3, standard: 4, deep_dive: 10, project_quest: 12 };
...
export async function generateLessonPlan(topic, learnerProfile, options = {}) {
  ...
  const system = [
    isQuest
      ...
      : isTip
      ? `You design a 60-SECOND QUICK TIP on: "${topic}" — a ${shape.label}. This is the SHORTEST format: a single bite-sized insight the learner can grasp and apply right away. Write EXACTLY 3 steps: one short "teach" step (the single insight — punchy, no depth), then one "activity" step that checks that insight, then the "recap". Exactly 1 objective. Do NOT pad it out.`
