...
export async function generateTopicRefinement(topic, messages, learnerProfile) {
  try {
    ...
    const system = [
      ...
      TOPIC_SPECIFICITY_RUBRIC,
      'Ask ONE short, focused question at a time to narrow it down. Keep questions friendly and concrete, and push toward a real task/problem/outcome (which tool? what are they trying to produce? what is going wrong?).',
      ...
      answersGiven >= 3
        ...
        : 'Only return done:true once you can name a clear, SPECIFIC, concrete lesson topic that fits what they want.',
      'When still gathering, return {"done": false, "message": "<your next question>"}.',
      'When ready, return {"done": true, "message": "<one-line confirmation of what the new lesson will cover>", "newTopic": "<the sharpened, self-contained, concrete lesson topic>"}.',
    ...
    return { done: false, message: 'Can you tell me a bit more about what you were hoping to learn?', newTopic: null };
  }
}
...
export async function generateLessonPlan(topic, learnerProfile, options = {}) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastError = error;
      console.error(`Lesson plan attempt ${attempt} failed:`, error.message);
    }
  }
...
export async function generateTeachStep(topic, learnerProfile, options = {}) {
  ...
  const system = [
    ...
    '- Flashcards — a term/trigger on the front, flip to reveal its definition + a quick example: {"type":"flashcards","title":"optional","cards":[{"front":"A trigger","back":"definition + a quick example"}]} (2-5 cards).',
...
export async function generateLessonAnswer(topic, learnerProfile, options = {}) {
  ...
  const qaBlock = Array.isArray(recentQa) && recentQa.length
    ...
    : '';
  const system = [
    `A learner is partway through a lesson on "${topic}" and asked a question. You DO have the lesson so far — it is provided below. Answer using it; never say you lack the history or context.`,
    'Answer helpfully and concisely (2-4 sentences), tying back to what the lesson already taught. Do NOT invent new exercises or activities — just answer the question.',
