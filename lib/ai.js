...
import { allowedSearchDomains, isApprovedSource } from './doc-sources';
...
// The pages the model actually opened, pulled out of the web_search result blocks
// so a lesson can show its sources.
//
// Read from the RESPONSE, never asked for in the JSON: a model asked to "list your
// sources" writes plausible-looking URLs from memory, and a fabricated citation is
// worse than none — it looks authoritative and sends people to a 404. If the search
// tool didn't run, this is empty and the lesson simply shows no sources.
function extractSources(response) {
  ...
  for (const block of response?.content || []) {
    ...
    for (const result of block.content) {
      ...
      out.push({
        title: (typeof result?.title === 'string' && result.title.trim()) || host || url,
        url,
        host,
      });
    }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      return {
        ...
        // Real pages from this response's search results — see extractSources.
