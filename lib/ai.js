...
import { AI_COMPETENCIES, COMPETENCY_KEYS, rubricText } from './ai-competencies';
...
// The six skills the calibration measures. All are generated around the person's
// real work; privacy is generated too (role-framed) but always has a vetted
// curated fallback since it's compliance-sensitive. Descriptions frame each skill
// as APPLIED to the learner's job, not as abstract AI knowledge.
const GENERATED_SKILLS = {
  privacy: 'handling sensitive/customer data responsibly when using AI for their work',
  prompting: 'getting good results from AI on their real tasks (structuring the ask, multi-step work)',
  comms: 'using AI to communicate with the people they actually deal with (customers, stakeholders, teammates)',
  eval: 'checking AI output on their work — catching mistakes and hallucinations before it ships',
  agents: 'automating parts of their own workflow and knowing when to trust automation',
  data: 'using AI on the data/reports they actually produce, and sanity-checking the numbers',
};
...
function buildCalibrationScenariosPrompt(learnerProfile) {
  ...
  return [
    ...
    'GOAL: gauge how well THIS specific person applies AI to THEIR OWN JOB — so we pitch their lessons right. You are NOT testing abstract/textbook AI knowledge (no "what is a token", no trivia). You ARE testing applied judgment on their real work.',
    '',
    'Return EXACTLY 6 scenarios as a JSON array — one per skill, in this order: privacy, prompting, comms, eval, agents, data.',
    'Each scenario object has:',
    '- id (string): the skill key (privacy | prompting | comms | eval | agents | data), each used exactly once',
    '- primary (string): same as id',
    '- setup (string): 1-2 plain sentences (~25 words max) describing a SPECIFIC, realistic moment from THIS PERSON\'S actual work — use their real tasks/role; name the kind of thing they\'d actually be doing. Plain language, not a word problem. No scene-setting fluff.',
    '- prompt (string): one short question (e.g. "How would you approach this?")',
    '- answers (array): EXACTLY 4 objects, each { "text": string, "scores": { <skillKey>: number } }. Each "text" is ONE short sentence (~14 words max), and ALL FOUR options must be roughly the SAME length — scannable at a glance, never multi-sentence.',
    '',
    'What each skill looks like FOR THIS PERSON (measure it through their work, not in the abstract):',
    skillLines,
    '',
    'Framing rules (most important):',
    '- BE BRIEF. A busy senior person should read the setup and all 4 options in under ~20 seconds. Long, wordy questions and multi-sentence answers feel like a chore and people bail — keep every line tight and scannable.',
    '- Each setup must read like it was written for someone with this exact role/tasks. A stranger should be able to guess their job from the scenarios.',
    '- The 4 options are different ways THEY might handle it with AI — from a naive approach to how a sharp, time-pressed expert in their role would actually do it. Every option must be genuinely plausible for this person; no obviously-wrong throwaways.',
    '- CRITICAL: never telegraph the best answer by length or wording. The expert option must NOT be the longest, most-detailed, or most-hedged. Keep all four the same length and phrasing style so which one is "right" is not guessable from how it reads.',
    '- Do not assume the person has lots of spare time. Senior people are slammed with meetings; the best answer is often the smart, time-efficient move (e.g. have AI draft something to gather input first), not the most elaborate effort.',
    '- Privacy scenario: set it in their own work with the real kind of data they touch. The best answer keeps sensitive data minimal/anonymized even on approved tools; the worst over-shares. Keep the "right" answer aligned with responsible data handling.',
    '',
    'Scoring rubric (keep consistent):',
    '- Every answer MUST score its scenario\'s primary skill, graduated across the 4 options: naive (~0.1-0.2), partial (~0.35-0.5), solid (~0.6-0.8), expert (~0.9-1.0).',
    `- scores keys must be from: ${VALID_SKILL_KEYS.join(', ')}. Values 0.0-1.0. An answer may add a small secondary score (0.2-0.5) on another skill.`,
    '- VARY which option is best across the 6 scenarios (don\'t always put it last), and vary its length independent of correctness — sometimes the best answer is the shortest one.',
    '',
    'Output rules:',
    '- Return ONLY the JSON array. No markdown fences, no prose.',
    tier ? `- Pitch difficulty to their level: ${tier}. For advanced/developer levels skip basics and probe real depth; for beginners keep it approachable.` : null,
    title ? `- Their title: ${title}.` : null,
    department ? `- They work in ${department}${sub_team ? ` (${sub_team})` : ''}.` : null,
    tasks ? `- Their day-to-day tasks: ${tasks}. EVERY scenario must be anchored in one of these.` : '- No task list given; infer plausible day-to-day work from their title/department and anchor scenarios there.',
    goal ? `- Their stated goal: ${goal}.` : null,
  ].filter(Boolean).join('\n');
}
...
// Generate role-tailored scenarios for all six skills. Returns a map
// { [skillKey]: scenario } containing only the ones that generated cleanly — the
// caller fills any gaps from the curated fallback set. Never throws.
// Uses Haiku (fast) so it reliably finishes inside the calibration gate's wait
// window; Sonnet's 20-40s runs were timing out and silently falling back.
export async function generateCalibrationScenarios(learnerProfile) {
  try {
    const response = await getClient().messages.create({
      ...
      messages: [{ role: 'user', content: 'Generate my 6 calibration scenarios.' }],
    });
    let text = response.content[0].text.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '');
    let parsed;
    try {
    ...
    for (const item of parsed) {
      ...
      if (!GENERATED_SKILLS[key] || byKey[key]) continue; // unknown skill / duplicate
...
// Synthesize the four AI Impact competency scores from the learner's answers.
// `entries` = [{ dimension, selfLevel, mcLabel, exampleText }] for each of
// personal|team|org|development. `calibrationSummary` is optional context the
// model may reference in the "why" (it does NOT change the number).
// Returns { [dimension]: { measured (1-5), why } }. Falls back to the self-claim
// with an empty why on any failure, so scoring never blocks the flow.
export async function generateImpactScores(learnerProfile, entries, calibrationSummary = '') {
  const byDim = {};
  for (const e of Array.isArray(entries) ? entries : []) {
    if (e && COMPETENCY_KEYS.includes(e.dimension)) byDim[e.dimension] = e;
  }
  const fallback = () => Object.fromEntries(
    COMPETENCY_KEYS.map((k) => [k, { measured: clampLevel(byDim[k]?.selfLevel ?? 3), why: '' }]),
  );

  const system = [
    'You assess a person\'s AI competency for an internal learning platform, using the company\'s official 1-5 rubric.',
    'For each competency: read their self-claim and their example, then assign the MEASURED level (1-5) that best fits the rubric based on the EVIDENCE they gave — not just what they claimed. A confident claim with a thin or empty example cannot justify a high score; lean on the lower/middle rubric levels when evidence is weak.',
    'Then write a short "why" (1-2 sentences, plain language, address them as "you") that justifies the score by pointing to what they actually said. Be fair, specific, and encouraging about the next step.',
    'Return ONLY JSON, no prose/fences, shaped exactly: {"personal":{"measured":<1-5>,"why":"..."},"team":{...},"org":{...},"development":{...}}.',
  ].join('\n');

  const sections = COMPETENCY_KEYS.map((k) => {
    const e = byDim[k] || {};
    return [
      `### ${AI_COMPETENCIES[k].label} (key: ${k})`,
      'Rubric:',
      rubricText(k),
      `Their self-claim: "${e.mcLabel || '(none)'}" (self level ${e.selfLevel ?? '?'})`,
      `Their example: ${e.exampleText ? `"${e.exampleText}"` : '(none given)'}`,
    ].join('\n');
  }).join('\n\n');

  const userContent = [
    calibrationSummary ? `For context only (may inform the "why", not the score): ${calibrationSummary}` : null,
    sections,
    'Score all four competencies now.',
  ].filter(Boolean).join('\n\n');
  ...
  try {
    ...
    for (const k of COMPETENCY_KEYS) {
      const r = parsed[k];
      out[k] = (r && r.measured !== undefined)
        ? { measured: clampLevel(r.measured), why: typeof r.why === 'string' ? r.why.trim() : '' }
        : { measured: clampLevel(byDim[k]?.selfLevel ?? 3), why: '' };
    }
    return out;
  } catch (error) {
...
export async function generateLessonPlan(topic, learnerProfile, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // The primary tool this lesson was built around (best of the learner's own
      // tools for the topic) — the reader reorders its tools to match and points
      // the "Open" button at it.
      return { headline, objectives, steps: capped, primaryTool: tools[0]?.id || null };
    } catch (error) {
      lastError = error;
