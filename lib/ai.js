...
export async function generateTeachStep(topic, learnerProfile, options = {}) {
  const { objectives = [], step = {}, priorTitles = [], priorContent = [] } = options;
  const { tier, department, top_tasks } = learnerProfile || {};
  const isDevTier = tier === 'developer';
  const tools = resolveTools(learnerProfile);
  const catalog = options.catalog || (await getMergedTools());
  ...
  const system = [
    ...
    '- Format for readability: use real newlines. Multi-step directions go in a numbered markdown list (each step on its own line: "1. ...\\n2. ..."), lists use "- " bullets, emphasis uses **bold**. Never cram steps into one run-on line.',
