...
import { buildToolGuidance, resolveTools, generalToolsOf } from './ai-tools';
import { getMergedTools } from './ai-tools-store';
import { getModelLineup, formatLineupForPrompt, validateModelForTool } from './model-lineup';
import { adaptiveGuidance } from './adaptive-level';
import { buildSignalBlock } from './learner-signals';
import { fillSuggestions } from './suggestion-filter';
import { allowedSearchDomains } from './doc-sources';
import { resolveUsedSources } from './lesson-sources';
import { parseJsonObject, describeRaw } from './json-extract';
import { planServerBudgetMs, PLAN_MIN_ATTEMPT_MS } from './lesson-timing';
import Anthropic from '@anthropic-ai/sdk';

const MODEL = MODELS.sonnet;

// The SDK's own defaults are a 10-MINUTE request timeout and TWO silent retries
// with backoff. Both are wrong for this app: every route here caps out between
// 30s and 300s, so a 10-minute socket can only ever be killed by the platform
// mid-call, and the hidden retries mean one "attempt" in our own retry loops can
// really be three API calls. During any Anthropic 429/529 blip that turns a 60s
// generation into a multi-minute one with nothing in our logs to show why
// (feedback #232). One retry, and a ceiling under the longest route.
const CLIENT_TIMEOUT_MS = 240_000;

let client;
function getClient() {
  if (!client) {
    client = new Anthropic({ timeout: CLIENT_TIMEOUT_MS, maxRetries: 1 });
  }
  return client;
}
...
// `extraContext` is appended verbatim at the end of the system prompt. It exists
// for the Slack coach, which knows something the in-app chat doesn't need to be
// told: what today's pick is, and how far through it the learner already is. In-app
// callers omit it and get the exact prompt they had before.
export function buildChatSystemPrompt(learnerProfile, catalog, lineupText = '', extraContext = '') {
  ...
  return [
    ...
    buildToolGuidance(resolveTools(learnerProfile), catalog, lineupText),
    !isDevTier
    ...
    // The visible reply and the tag are written in one pass, so nothing stopped
    // the model asking "what specifically do you want to work on?" while ALSO
    // naming a topic — which made the app staple a "Start a lesson" offer onto a
    // question asking the learner to narrow down. Testers read that as the coach
    // contradicting itself: told to be more specific, handed a lesson anyway
    // (feedback #198).
    '- If your visible reply ASKS THE LEARNER A CLARIFYING QUESTION (anything like "what specifically...", "which part...", "can you tell me more?"), you MUST use [[LESSON_TOPIC: NONE]]. Offering a lesson on a topic you just said you need more detail about contradicts the question you asked. Wait for their answer, then offer the lesson on your next reply.',
    '- Never mention, explain, or format this tag in your visible answer, and never write anything after it.',
    // Appended last so it can't be buried by the rules above, and only when a
    // caller supplied it.
    extraContext ? `\n${extraContext}` : null,
  ].filter(Boolean).join('\n');
}

// Is this reply asking the learner to narrow their request down? Both conditions
// are required: a question mark, and a phrase that is unambiguously a request for
// more detail. See the note at the call site.
const NARROWING_PHRASES = /\b(more specific|be specific|what specifically|which specific|can you tell me more|a bit more detail|more detail about|narrow (it|this|that) down|what (part|kind|sort) of)\b/i;

function asksToNarrow(reply) {
  const text = String(reply || '');
  return text.includes('?') && NARROWING_PHRASES.test(text);
}

export async function generateChatReply(messages, learnerProfile, options = {}) {
  try {
    const catalog = await getMergedTools();
    const lineupText = formatLineupForPrompt(await getModelLineup(), resolveTools(learnerProfile).map((t) => t.id));
    ...
    const response = await getClient().messages.create({
      model: MODEL,
      max_tokens: 450,
      system: buildChatSystemPrompt(learnerProfile, catalog, lineupText, options.extraContext || ''),
    ...
    // Belt and braces for the rule above: a prompt rule is a request, and this one
    // produces a visibly self-contradicting message when it is ignored. If the
    // reply is clearly asking the learner to narrow down, drop the lesson offer
    // regardless of what the tag said. Deliberately conservative — it needs an
    // actual question mark AND an explicit narrowing phrase — so a normal answer
    // that happens to contain the word "specific" still gets its offer.
    if (lessonTopic && asksToNarrow(reply)) lessonTopic = null;
...
export function buildHelpSystemPrompt() {
  return [
    ...
    '  - HCPMX Skill Shop Help (MX team): https://housecall.slack.com/archives/C04J8BRUJQY',
  ].join('\n');
}
...
export function buildSuggestedTopicsPrompt(learnerProfile, options = {}) {
  const { department, sub_team, tier } = learnerProfile || {};
  const tierLabel = tier || 'beginner';
  ...
  return [
    ...
    '- Spread the six across their tasks, their goals AND their projects below — not six angles on one task.',
    '',
    // Was a tasks line plus a bare goal line, with projects absent entirely. The
    // picker's cache is keyed on all four signals now, so a project or goal edit
    // regenerates this list — it has to actually READ them or the new list comes
    // back the same as the old one.
    buildSignalBlock(learnerProfile),
  ].filter(Boolean).join('\n');
}
...
export async function generateSuggestedTopics(learnerProfile, options = {}) {
  try {
    ...
    // The prompt asks the model to skip completed lessons; this enforces it and
    // tops the list back up to six. See lib/suggestion-filter.js.
    return fillSuggestions(parsed, options.exclude);
  } catch (error) {
...
// ---------------------------------------------------------------------------
// AI Impact scoring (synthesized against the company competency rubric)
// ---------------------------------------------------------------------------
...
export async function generateToolRecommendation(topic, options = {}) {
  try {
    ...
    // When the caller passes the learner's OWN tools, choose only from those (so
    // the lesson is built around a tool they actually have); otherwise the whole
    // catalog is fair game.
    const source = options.allowedTools?.length ? options.allowedTools : catalog;
    const list = source
      .map((t) => `- ${t.label}${t.strengths ? `: best for ${t.strengths}` : ''}`)
      .join('\n');

    // Pick the MODEL here too, in the same call that picks the tool. Two reasons:
    // the topic is known at exactly this point, and doing it once means the
    // on-screen hint and the generated lesson prose name the same model instead
    // of drifting apart (the lesson prompt is handed this pick — see below).
    const lineup = await getModelLineup().catch(() => null);
    const lineupRows = (lineup?.tools || [])
      .map((t) => {
        const tiers = [
          t.fast ? `quick: ${t.fast}` : null,
          t.balanced ? `everyday: ${t.balanced}` : null,
          t.reasoning ? `deep reasoning: ${t.reasoning}` : null,
        ].filter(Boolean).join(', ');
        return tiers ? `- ${t.label} — ${tiers}` : null;
      })
      .filter(Boolean)
      .join('\n');
    ...
    const system = [
      ...
      ...(lineupRows
        ? [
          '',
          'ALSO pick which MODEL inside that tool the learner should use for this topic.',
          'These are the current models per tool:',
          lineupRows,
          'Rules for the model pick:',
          '- The model MUST belong to the tool you recommended. Never name a model from a different tool, and never invent one.',
          '- Match the model to the work: the deep-reasoning model for multi-step, high-stakes, or analytical topics; the quick model when the task is simple or high-volume. Do NOT default to the deep-reasoning model for everything.',
          '- modelWhy is ONE short plain sentence tied to this topic, e.g. "this one is worth the wait because the output goes in front of a customer".',
        ]
        : []),
      lineupRows
        ? 'Return ONLY JSON: {"tool": "<exact tool label from the list>", "why": "<one short, plain sentence on why it fits THIS topic>", "model": "<exact model name from the lineup for that tool>", "modelWhy": "<one short plain sentence>"}.'
    ...
    const response = await getClient().messages.create({
      ...
      max_tokens: 400,
      system,
      messages: [{ role: 'user', content: `Lesson topic: "${topic}". Which tool is best, which model inside it, and why?` }],
    });
    let text = response.content[0].text.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '');
    const parsed = JSON.parse(text);
    ...
    // Only keep a model that really is one of the recommended tool's models. A
    // hallucinated or cross-tool name would tell the learner to switch to
    // something that isn't in their tool's picker.
    const model = validateModelForTool(lineup, tool, parsed.model);
    const modelWhy = model ? String(parsed.modelWhy || '').trim() : '';
    return { tool, why, ...(model ? { model, modelWhy } : {}) };
  } catch (error) {
...
// Reorder the learner's OWN tools so the single best one for THIS topic is first
// (which every generator treats as the primary tool). This is how the coach
// "picks the best of your tools for the topic" — falls back to the given order
// on any failure. Never introduces a tool the learner didn't select.
export async function orderToolsByTopic(topic, tools) {
  if (!Array.isArray(tools) || tools.length <= 1) return tools;
  try {
    const rec = await generateToolRecommendation(topic, { allowedTools: tools });
    const pick = (rec?.tool || '').toLowerCase();
    if (!pick) return tools;
    const idx = tools.findIndex(
      (t) => t.label?.toLowerCase() === pick || t.id?.toLowerCase() === pick,
    );
...
// How many answers the learner gives before refine has to commit to a topic.
// Enforced in code below, not just asked for in the prompt.
const MAX_REFINE_ANSWERS = 3;
...
export async function generateTopicRefinement(topic, messages, learnerProfile) {
  try {
    ...
    const system = [
      ...
      answersGiven >= MAX_REFINE_ANSWERS
    ...
    const question = parsed.message ? String(parsed.message).trim() : '';

    // The answersGiven line in the system prompt is only advice, and Haiku is free
    // to ignore it. Nothing else ended this loop, so a learner could be asked
    // question after question with no way out (feedback #182) — cap it here. If the
    // model still won't commit to a topic, the learner's own last answer is the
    // best signal we have, and they confirm the topic before anything rebuilds.
    if (!done && answersGiven >= MAX_REFINE_ANSWERS) {
      const lastAnswer = [...(messages || [])].reverse().find((m) => m?.role === 'user' && m.content);
      const inferred = (parsed.newTopic ? String(parsed.newTopic).trim() : '')
        || (lastAnswer ? String(lastAnswer.content).trim() : '');
      if (inferred) {
        return { done: true, message: 'Got it — here\'s what I\'ll rebuild this around.', newTopic: inferred, tool: tool || null };
      }
    }

    // No question to ask and no topic to hand back is a failure, not a turn in the
    // conversation. Report it as one. Emitting a canned question here is what made
    // a broken refine indistinguishable from a real one, turn after turn.
    if (!done && !question) {
      return { error: true, done: false, message: null, newTopic: null, tool: null };
    }

    return {
      done,
      message: question || 'Here\'s a better-fitting lesson.',
...
export function buildLessonSystemPrompt(topic, learnerProfile, options = {}) {
  const { display_name, department, sub_team, tier } = learnerProfile || {};
  const tierLabel = tier || 'beginner';
  const isDevTier = tierLabel === 'developer';
  const format = options.format || 'standard';
  const tools = resolveTools(learnerProfile);
  const primaryTool = tools[0];
  // With several tools there is NO default — the coach picks the best one for the
  // task. So anywhere a single tool name would otherwise be written, write this
  // instead; naming tools[0] would quietly turn tick-box order into a default.
  const toolPhrase = tools.length > 1 ? 'the tool you pick for this lesson' : primaryTool.label;
  ...
  const formatGuide = {
    quick_tip: [
      ...
      `  2. ONE ready-to-use AI prompt they can copy and paste into their AI tool (${toolPhrase}) right now, shown in a fenced code block, with a one-line note on what it does.`,
  ...
  // Tasks, goals AND projects — see lib/learner-signals.js. This used to be a
  // lone tasks line, which is why goals never shaped a lesson.
  const signalBlock = buildSignalBlock(learnerProfile);
  ...
  return [
    ...
    tierBlock,
    // Fine-tune depth from how the learner has actually been performing recently
    // (empty until we have graded activities to go on).
    adaptiveGuidance(learnerProfile?.performance_band, learnerProfile?.adaptive_samples),
    '',
    ...
    buildToolGuidance(tools, options.catalog, options.lineupText),
    ...
    `- BE IN THE SOFTWARE: tell the learner up front to open their AI tool (${toolPhrase}) in a separate window and follow along live. If the lesson also involves a tool that isn't theirs to do AI work in (a spreadsheet, Slack, a CRM), have them open that too. They do the actual AI work in their tool as you coach them.`,
    ...
    isDevTier ? null : `- REMINDER: the learner level above is binding. No code or jargon — reframe technical topics in plain English. Practice happens in their AI tool (${toolPhrase}), with prompts you hand them to paste in.`,
    display_name ? `- The learner's name is ${display_name}.` : null,
    department ? `- They work in ${department}${sub_team ? ` (${sub_team})` : ''}.` : null,
    '',
    signalBlock,
...
export async function generateLessonResponse(topic, messages, learnerProfile, options = {}) {
  ...
  try {
    ...
    const response = await getClient().messages.create({
      ...
      system: buildLessonSystemPrompt(topic, learnerProfile, { ...options, catalog, lineupText }),
      messages,
    });
...
export async function generateLessonQuiz(topic, transcript, learnerProfile, options = {}) {
  ...
  const system = [
    ...
    'Keep questions and options concise (one sentence each). Keep all 4 options roughly the SAME length — never make the correct one the longest or most detailed, and vary which position is correct, so the answer can\'t be guessed from length or wording.',
...
const PLAN_SHAPE = {
  quick_tip: { objectives: '1', label: 'Quick Tip (60-second insight)', steps: '2' },
  standard: { objectives: '1-2', label: 'Quick Lesson (3-5 min)', steps: '2-4' },
  deep_dive: { objectives: '2-4', label: 'Deep Dive (15-20 min)', steps: '6-8' },
  project_quest: { objectives: '3-5', label: 'Project Quest (20-60 min)', steps: '6-8' },
};
...
const PLAN_MAX_STEPS = { quick_tip: 2, standard: 4, deep_dive: 8, project_quest: 8 };

// Output-token CEILING per format — a cap, not a target. Latency comes from the
// tokens actually emitted, which PLAN_MAX_STEPS above governs; raising a ceiling
// the model doesn't reach costs nothing. So keep these generous.
//
// Trimming them alongside the step counts was a mistake worth not repeating: a
// project_quest plan embeds brief + deliverableName + example + reviewFocus for
// every build step (far more verbose per step than an activity spec), so 5000
// truncated the JSON mid-object on a demanding topic, JSON.parse failed, both
// retries failed the same way, and the learner got "Failed to design the lesson"
// after ~3 minutes. Deep Dive survived the same cap only because activity specs
// are smaller. Headroom here is free; the step cap is the real latency lever.
const PLAN_TOKENS = { quick_tip: 2500, standard: 4500, deep_dive: 6500, project_quest: 8000 };
...
// Feedback #221: an automated-evals lesson handed out a sample judge prompt that
// scored a trace on a 1-5 Likert scale. Likert scoring is widely discouraged for
// LLM-as-judge work — the middle of the scale means something different every run —
// and boolean or explicitly-defined criteria are the current recommendation. Left
// unsaid, the model reaches for a 1-5 scale because so much older writing does.
//
// Only attached to lessons that are actually about evaluation, so the other prompts
// don't carry a rule they'll never use.
const EVAL_TOPIC_TOKENS = [
  'eval', 'evaluation', 'scoring', 'score', 'judge', 'rubric', 'regression',
  'accuracy', 'quality check', 'langsmith', 'hallucination',
];

function evalScoringGuidance(topic, objectivesText = '') {
  const haystack = `${topic || ''} ${objectivesText}`.toLowerCase();
  if (!EVAL_TOPIC_TOKENS.some((k) => haystack.includes(k))) return null;
  return [
    'SCORING (this lesson touches evaluation — follow this):',
    '- Use BOOLEAN (pass/fail) or exact-match criteria, or a short list of discrete yes/no checks. That is what current practice recommends.',
    '- Do NOT hand the learner a 1-5 or 1-10 Likert-style scale for an LLM-as-judge prompt. Vague scales score the same output differently run to run, and the industry has moved away from them.',
    '- If a numeric score genuinely cannot be avoided, define what EVERY level means in concrete, checkable terms — never "3 = okay".',
    '- Prefer several narrow checks ("Does it cite the policy? yes/no", "Does it give a next step? yes/no") over one broad quality number.',
  ].join('\n');
}

// Anthropic server-side web search tool spec (executed by the API in-request).
// allowed_domains restricts grounding to vendor documentation, official help
// centres and the vendors' own community forums — see lib/doc-sources.js for why.
// Enforced at SEARCH time, so the model cannot read a third-party blog even if it
// wants to, rather than being asked nicely not to cite one.
...
export async function generateLessonPlan(topic, learnerProfile, options = {}) {
  const format = ['quick_tip', 'deep_dive', 'project_quest'].includes(options.format)
    ? options.format : 'standard';
  const shape = PLAN_SHAPE[format];
  const { tier, department } = learnerProfile || {};
  const isDevTier = tier === 'developer';
  // Pick the best of the learner's tools for THIS topic and make it primary, so
  // the whole lesson (prose, prompts, the Open button) is built around it.
  // These three are independent (a tool ordering, a catalog read, a lineup read)
  // and used to run one after another, stacking three round trips onto the front
  // of every lesson before the plan call even started. Fetch them together.
  const [tools, catalog, lineup] = await Promise.all([
    orderToolsByTopic(topic, resolveTools(learnerProfile)),
    options.catalog ? Promise.resolve(options.catalog) : getMergedTools(),
    getModelLineup(),
  ]);
  let lineupText = formatLineupForPrompt(lineup, tools.map((t) => t.id));
  // The model was already chosen for this topic when the tool was (and the
  // learner can see it on screen). Pin the prose to that exact pick so the lesson
  // can't tell them one model while the banner tells them another.
  if (options.recommendedModel) {
    lineupText = [
      lineupText,
      `THE MODEL FOR THIS LESSON IS ALREADY DECIDED: ${options.recommendedModel}. The learner is being shown that name on screen. When you name a model for the hands-on work, name EXACTLY "${options.recommendedModel}" — do not substitute a different model or tier.`,
    ].filter(Boolean).join('\n');
  }

  const isQuest = format === 'project_quest';
  const isTip = format === 'quick_tip';
  const system = [
    isQuest
      ...
      : isTip
      ? `You design a 60-SECOND QUICK TIP on: "${topic}" — a ${shape.label}. This is the SHORTEST format: a single bite-sized insight the learner reads and applies right away. It is READ-ONLY — NO quiz, NO exercise. Write EXACTLY 2 steps: one short "teach" step (the tip itself — punchy, no depth) then the "recap". Exactly 1 objective. Do NOT add an activity and do NOT pad it out.`
      : `You design a hands-on AI micro-lesson on: "${topic}" — a ${shape.label}.`,
    '',
    'Also write a "headline": ONE plain-language sentence (about 12-20 words) that describes what this lesson helps the learner DO — a friendly summary of the whole lesson, shown at the top above the objectives. Same plain-language rules as objectives (NO leverage/utilize/optimize/facilitate/synthesize and no corporate words); write like you would say it to a coworker. It should read as a natural sentence, not a list. Example: "Use AI to draft and tighten a training outline from a role and a goal."',
    '',
    'LEARNING OBJECTIVES ARE THE SOURCE OF TRUTH. Write them first, then build everything to serve them.',
    `Write ${shape.objectives} objectives — choose the count based on how much THIS topic genuinely needs, not a default you always reach for. A narrow, simple topic should land at the LOW end of that range; a topic covering several distinct skills or a lot of ground should land at the HIGH end. Two different topics in the same format should NOT usually produce the same objective count. Each objective MUST:`,
    ...
    `- Aim for ${shape.steps} steps total, scaling with how many objectives you chose above (more objectives means more steps, within this range), and NEVER exceed ${PLAN_MAX_STEPS[format] || PLAN_MAX_STEPS.standard} steps including the recap. Alternate short teaching steps with hands-on steps.`,
    isQuest
      ? [
      ...
      : isTip
      ? '- READ-ONLY tip: do NOT include ANY "activity" step. Output EXACTLY the ONE "teach" step (the tip) followed by the "recap" — nothing else.'
    ...
    // Which activity type got used was previously left entirely to the model, and
    // the JSON example below shows "mcq" — so lessons drifted to all-multiple-
    // choice and learners stopped being asked to actually WRITE anything. This
    // platform's whole premise is doing the work in your own AI tool, and "write"
    // is the only type that proves they can. So require one.
    isQuest || isTip
      ? null
      : '- REQUIRED: at least ONE activity in this lesson MUST be activityType "write" — the learner produces something real (a prompt, a draft, a message) and gets it graded. This is the only type that proves they can DO the work rather than recognise the right answer, so a lesson without one is incomplete. If the lesson has room for only one activity, make it the "write" one. Use the other types for ADDITIONAL activities beyond that.',
    ...
    '- ANSWER OPTIONS (mcq / scenario): keep all options roughly the SAME length and phrasing style. NEVER make the correct option the longest or most detailed one, and vary which position is correct — a learner must not be able to guess the answer from length or wording alone. Put any extra reasoning in optionFeedback/explanation, not in the correct option\'s text.',
    ...
    adaptiveGuidance(learnerProfile?.performance_band, learnerProfile?.adaptive_samples) || null,
    ...
    buildSignalBlock(learnerProfile),
    '',
    buildToolGuidance(tools, catalog, lineupText),
    '',
    'Return ONLY JSON (no fences):',
    '{',
    '  "headline": "one plain-language sentence describing what this lesson helps them do",',
    // Only the pages that actually shaped the lesson. Cross-checked against the real
    // search results before anything is shown, so an invented URL is dropped — but
    // the point of asking is the opposite direction: without it we would list every
    // page the search returned and credit documents the lesson never used.
    '  "sourcesUsed": ["exact URLs, copied character-for-character from the search results, of the pages you actually drew facts from. Leave it [] if you did not use any — an empty list is a correct answer and far better than listing pages you did not read. Never write a URL that was not in the search results."],',
    // Which of the learner's own tools this lesson is built around. The learner
    // has no default tool by design — the coach picks the best one for the task —
    // so the CHOICE has to come back from here. Without it the code fell back to
    // whichever tool happened to be first in their list, which is just the order
    // they ticked the boxes in.
    `  "tool": "the ONE tool from the learner's own list this lesson is built around — pick the best fit for THIS topic, exactly as written: ${tools.map((t) => t.label).join(' | ')}",`,
    ...
    isQuest
      ...
      : isTip
      ? null
      // Shows "write" deliberately: this template is the strongest signal in the
      // whole prompt, and when it said "mcq" plans copied it.
      : '    { "id": "s2", "kind": "activity", "objectiveId": "o1", "activityType": "write", "activity": { ...spec for that type... } },',
    '    { "id": "sN", "kind": "recap", "title": "Recap" }',
    '  ]',
    '}',
    'The LAST step must be kind "recap". The FIRST step must be kind "teach" (an intro).',
    evalScoringGuidance(topic),
    // Every lesson is grounded now, not just niche-product ones. A lesson written
    // purely from model memory is how feedback #221 happened: an evals lesson
    // taught a 1-5 Likert judge prompt, which current practice discourages, and
    // nothing checked it against the tool's own docs. Searching also produces the
    // citations the lesson shows the learner — and those have to come from pages
    // actually read, never from recall.
    `\nGROUND THIS LESSON IN REAL SOURCES — do this FIRST, before writing anything:`,
    '- Use the web_search tool to check how this actually works today. Search is restricted to official vendor documentation, official help centres, and the vendors\' own community forums — third-party blogs and tutorials are not reachable, by design.',
    '- Prefer the docs themselves. Use a community/forum thread when it answers something the docs do not (a real limit, a known error, a workaround), not as a substitute for reading the docs.',
    '- The learner is shown the sources you list in "sourcesUsed" as "what this lesson was built from". List ONLY pages you genuinely took something from. A page you searched past, or skimmed and did not use, does not belong there.',
    '- Build the objectives and steps around what you find. Where the docs and your prior knowledge disagree, the docs win — they are current and you may not be.',
    '- Never invent product features, menu names, field names or limits. If you cannot confirm something, teach the part you can confirm and say plainly what to check in the tool.',
    looksLikeSpecificTool(topic)
      ? `- "${topic}" names a SPECIFIC, possibly recent AI product or feature, not a general skill. Do NOT substitute the parent brand or teach generic basics. If you genuinely cannot confirm it exists, say so plainly in the intro step and teach the closest real workflow.`
      : null,
  ].filter(Boolean).join('\n');
  ...
  // EVERY lesson is grounded with a live lookup, not just niche-product topics.
  // If the web-search tool errors (e.g. not enabled on the account), we drop it and
  // retry plainly so the lesson still generates — just without fresh grounding, and
  // without citations.
  let useWebSearch = true;
  let lastError;
  let lastRaw = '';
  const isHeavy = format === 'project_quest' || format === 'deep_dive';
  // Retries are bounded by TIME, not just by count.
  //
  // They used to be counted only — three attempts for a light format — which is
  // how a standard lesson could spend 204s failing (feedback #232): three ~60s
  // generations, while the browser gave up at 150s and told the learner it was
  // taking too long. Counting says "three tries"; the learner experiences
  // "three minutes". The deadline is what the CLIENT will wait minus a margin
  // (lib/lesson-timing.js), so the server always answers before anyone gives up.
  const startedAt = Date.now();
  const deadline = startedAt + planServerBudgetMs(format);
  const maxAttempts = isHeavy ? 2 : 3;
  let attempt = 0;
  while (attempt < maxAttempts) {
    attempt++;
    const attemptStart = Date.now();
    const remaining = deadline - attemptStart;
    // Never START an attempt that can't finish. A run killed mid-flight costs
    // the learner the full wait and produces nothing to show for it.
    if (attempt > 1 && remaining < PLAN_MIN_ATTEMPT_MS) {
      console.warn(`Lesson plan: out of time after ${attempt - 1} attempt(s) (${Math.round((Date.now() - startedAt) / 1000)}s), not starting another`);
      break;
    }
    try {
      const response = await getClient().messages.create({
        ...
        // Sonnet 4.6 defaults effort to "high" when this is omitted, which spends
        // more tokens (and time) than plan generation needs. Thinking is disabled
        // explicitly rather than by omission so a future model swap — Sonnet 5
        // turns adaptive thinking ON by default — can't silently add latency here.
        ...
        // Per-request, so the ceiling is what's actually left of the deadline
        // rather than the client-wide default. maxRetries 0 because this loop
        // owns retrying: an SDK retry underneath would spend the deadline
        // without the loop ever knowing it happened.
        timeout: Math.max(PLAN_MIN_ATTEMPT_MS, remaining),
        maxRetries: 0,
      });
      // A plan that hit its ceiling is cut off mid-object, so the parse below is
      // guaranteed to fail — and it used to fail as a generic "No JSON in plan
      // response", which reads like a model hiccup and sends you looking in the
      // wrong place. Name the real cause instead: this format's PLAN_TOKENS is
      // too low for the JSON it has to emit.
      if (response.stop_reason === 'max_tokens') {
        throw new Error(
          `Plan truncated: format="${format}" hit its ${PLAN_TOKENS[format] || PLAN_TOKENS.standard}-token ceiling. Raise PLAN_TOKENS.${format} or lower PLAN_MAX_STEPS.${format}.`,
        );
      }
      const text = responseText(response);
      lastRaw = text;
      // Ask for the object that IS a plan, not merely the first "{...}" in the
      // reply. The old greedy `/\{[\s\S]*\}/` fallback spliced together every
      // object the model emitted and then failed to parse the result — three
      // times in a row, since a retry changes what the model says but not how we
      // read it. That is the whole of feedback #232; see lib/json-extract.js.
      const parsed = parseJsonObject(text, {
        want: (o) => Array.isArray(o.objectives) && Array.isArray(o.steps),
      });
      if (!parsed) throw new Error(`No plan JSON in response (${describeRaw(text)})`);
      ...
      // Back the "at least one write activity" rule with an actual check, since a
      // prompt instruction alone is probabilistic. Spend a retry on it, but NEVER
      // fail the lesson over it — an all-multiple-choice lesson still teaches
      // something, and "Failed to design the lesson" teaches nothing.
      //
      // The retry is also the only place we throw away a plan that WORKS, so it
      // is the first thing to give up when time is short: a learner waiting out a
      // second full generation to upgrade one activity type is a worse outcome
      // than the lesson they already had.
      const needsWrite = !isQuest && !isTip;
      const hasWrite = capped.some((s) => s.kind === 'activity' && s.activityType === 'write');
      const timeForAnother = deadline - Date.now() >= PLAN_MIN_ATTEMPT_MS;
      if (needsWrite && !hasWrite && attempt < maxAttempts && timeForAnother) {
        throw new Error(`Plan has no "write" activity (format="${format}") — retrying`);
      }
      if (needsWrite && !hasWrite) {
        console.warn(`Lesson plan accepted without a "write" activity after ${maxAttempts} attempts (topic="${topic}", format="${format}")`);
      }
      // The tool this lesson was built around — the reader reorders its tools to
      // match and points the "Open" button at it.
      //
      // Taken from the model's own choice, matched back against the learner's
      // tools by label. This used to be tools[0], which made the FIRST tool a de
      // facto default and contradicted the whole design: there is no default, the
      // coach picks per task. Falls back to a chat tool (then the first tool) only
      // when the model named nothing usable.
      const named = typeof parsed.tool === 'string' ? parsed.tool.trim().toLowerCase() : '';
      const matched = tools.find((t) => t.label.toLowerCase() === named)
        || tools.find((t) => named && named.includes(t.label.toLowerCase()));
      const fallback = generalToolsOf(tools)[0] || tools[0];
      return {
        headline,
        objectives,
        steps: capped,
        primaryTool: (matched || fallback)?.id || null,
        // Only pages confirmed as USED — see resolveUsedSources.
        sources: resolveUsedSources(response, parsed.sourcesUsed),
      };
    } catch (error) {
      lastError = error;
  ...
  // Keep the learner-facing text friendly — it goes straight to the screen — but
  // carry the real cause on the error so the audit log records something you can
  // actually act on. Replacing lastError with this generic string is why feedback
  // #181 (a 258s project_quest failure) had no diagnosable cause: the audit entry
  // read "Failed to design the lesson", which names no mechanism at all.
  //
  // `raw` matters as much as the message. Feedback #232 survived several rounds
  // of fixes because the audit entry recorded a JSON.parse error and nothing
  // else: you could infer from the character offset that the model had emitted
  // two objects, but never actually SEE what it wrote. Now the reply's head is
  // in the log, and `attempts` reports what we really spent, not the ceiling.
  const err = new Error('Failed to design the lesson. Please try again.');
  err.cause = lastError;
  err.diagnostic = [
    `format=${format}`,
    `attempts=${attempt}/${maxAttempts}`,
    `elapsed=${Math.round((Date.now() - startedAt) / 1000)}s`,
    `budget=${Math.round(planServerBudgetMs(format) / 1000)}s`,
    `webSearch=${useWebSearch}`,
    `lastError=${lastError?.message || 'unknown'}`,
    `raw=${describeRaw(lastRaw)}`,
  ].join(' ');
  throw err;
}
...
// How deep each step's own body text should go — this is what actually
// determines reading time, independent of the plan's step COUNT (a Deep Dive
// with more steps that are each just as short as a Quick Lesson's still reads
// just as fast, so depth has to scale per format too).
const TEACH_DEPTH = {
  // Prose stays SHORT (see sentences), but the token budget must fit the full
  // JSON — concept + 1-2 interactive blocks + keyPoints — or the response
  // truncates mid-object and JSON.parse fails (route 500s, step never loads).
  quick_tip: { sentences: '2-3 short sentences — a single punchy insight, no depth or nuance; this is a 60-second tip', tokens: 1500 },
  standard: { sentences: '2-4 short sentences', tokens: 1600 },
  deep_dive: { sentences: '5-8 sentences — go a level deeper: cover the why behind it, a nuance or common mistake, not just the what', tokens: 2400 },
  project_quest: { sentences: '3-6 sentences, focused tightly on what they need to know to build this exact piece', tokens: 2000 },
};
...
export async function generateTeachStep(topic, learnerProfile, options = {}) {
  ...
  const format = ['quick_tip', 'deep_dive', 'project_quest'].includes(options.format) ? options.format : 'standard';
  const depth = TEACH_DEPTH[format] || TEACH_DEPTH.standard;
  const isTip = format === 'quick_tip';
  // `concise` is for surfaces that are read in a chat window rather than on a page —
  // Slack, today. The web player can afford a screenful with clickable blocks; a DM
  // cannot, and a step that arrived as ~2,900 characters of prose plus flattened
  // tab content was genuinely hard to follow. Same objectives, same activities,
  // just told at chat length.
  const isConcise = options.concise === true;
  const { tier, department } = learnerProfile || {};
  const isDevTier = tier === 'developer';
  const tools = resolveTools(learnerProfile);
  ...
  const upcomingLine = upcoming && (upcoming.objective || upcoming.activityType)
    ? [
        ...
        covers.length
          ...
          : null,
        // Testers found they could copy the worked example straight into the
        // activity and pass without writing anything (feedback #192). "Demonstrate
        // exactly the kind of task" and "hand them the answer" are one step apart
        // when the task is to produce a piece of text, so the gap is closed here.
        upcoming.activityType === 'write'
          ? 'CRITICAL: that activity asks the learner to WRITE the thing themselves. Your worked example must therefore use a DIFFERENT scenario, audience or subject than the activity will set — same technique, different case. Show the method and call out what makes it work, but never produce text that could be pasted into that activity as a passing submission. If your example would earn a good grade on the activity, rewrite it around another situation.'
          : null,
      ].filter(Boolean).join('\n')
    : '';
  const system = [
    ...
    isConcise
      ? [
          'CHAT FORMAT (important) — this step is read inside a Slack direct message, not on a web page:',
          '- Keep the WHOLE "message" to 2-4 SHORT sentences. One idea, stated plainly, plus ONE concrete example inline. No preamble.',
          '- Return an EMPTY "blocks" array []. No flashcards, reveals, tabs, or diagrams — they cannot be clicked in a DM.',
          '- No headings, no tables, no mermaid. At most ONE short bullet list of 2-3 items if it genuinely helps.',
          '- At most 2 "keyPoints", each under 12 words. Omit them entirely if the message already says it.',
          '- Do not restate anything in a different form. Length here is a cost, not a signal of effort.',
        ].join('\n')
      : isTip
      ? 'QUICK TIP FORMAT — this is a single READ-ONLY page: return the tip as PLAIN TEXT in "message" ONLY. Use an EMPTY "blocks" array [] and an EMPTY "keyPoints" array []. No flashcards, reveals, tabs, or diagrams. Just the insight in 2-3 short sentences with one concrete example inline.'
    ...
    `- Keep the "message" concept to ${isConcise ? '2-4 SHORT sentences total (chat-sized)' : depth.sentences}. Use real newlines, "- " bullets, **bold**. A \`\`\`mermaid flowchart LR\`\`\` (3-6 plain-text nodes, no punctuation in labels) or a markdown table is welcome in the message when a static visual helps — but if you use a clickable diagram block, do NOT also draw the same thing in mermaid.`,
    ...
    department ? `- They work in ${department}.` : null,
    '',
    // The sample judge prompt in feedback #221 was written HERE, in a teach step —
    // so the rule has to live here too, not only in the plan prompt.
    evalScoringGuidance(topic, objText),
    '',
    buildSignalBlock(learnerProfile),
    '',
  ...
  try {
    const response = await getClient().messages.create({
      ...
      // A teach step is short prose plus 1-2 blocks — it does not need the
      // "high" effort Sonnet 4.6 applies by default. See generateLessonPlan for
      // why thinking is disabled explicitly rather than by omission.
    ...
    return {
      message,
      // Quick Tip is a single read-only text page: force NO interactive blocks
      // and NO key points regardless of what the model returned, so it's one
      // panel with nothing gating "Continue" (models ignore the prompt's
      // "empty blocks" ask often enough that we enforce it here).
      // Enforced rather than requested: models return interactive blocks even when
      // told not to, and in a DM those get flattened into duplicated prose.
      blocks: (isTip || isConcise) ? [] : sanitizeTeachBlocks(parsed.blocks),
      keyPoints: isTip ? [] : (Array.isArray(parsed.keyPoints) ? parsed.keyPoints.slice(0, isConcise ? 2 : 3) : []),
    };
  } catch (error) {
...
export async function generateVideoScript(topic, learnerProfile, options = {}) {
  ...
  try {
    ...
    const response = await getClient().messages.create({
      model: MODEL,
      max_tokens: maxTokens,
      // Narrated scripts are linear spoken prose, not a structured plan, so they
      // don't need the "high" effort Sonnet 4.6 applies when output_config is
      // omitted. Same reasoning as generateLessonPlan for disabling thinking
    ...
    // Same guard as the lesson plan: a script cut off at its ceiling can't parse,
    // and the generic JSON error that follows points away from the real cause.
    // A narrated Project Quest is especially exposed, since it emits one scene per
    // curated quest step and so grows with the quest rather than with the format.
    if (response.stop_reason === 'max_tokens') {
      throw new Error(
        `Video script truncated: format="${format}" hit its ${maxTokens}-token ceiling. Raise VIDEO_FORMAT_TOKENS.${format}.`,
      );
    }
