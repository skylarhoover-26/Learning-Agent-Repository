...
export async function generateLessonPlan(topic, learnerProfile, options = {}) {
  ...
  let lastError;
  // Retries must FIT the route's maxDuration (300s) or the function is killed
  // mid-attempt and the learner sees a failure anyway. A heavy-format plan runs
  // ~90-120s, so two attempts fit and three do not; light formats are fast
  // enough for three.
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (needsWrite && !hasWrite && attempt < maxAttempts) {
