...
export async function generateTeachStep(topic, learnerProfile, options = {}) {
  ...
  const system = [
    ...
    `- Keep the "message" concept to ${depth.sentences}. Use real newlines, "- " bullets, **bold**. A \`\`\`mermaid flowchart LR\`\`\` (3-6 plain-text nodes, no punctuation in labels) or a markdown table is welcome in the message when a static visual helps — but if you use a clickable diagram block, do NOT also draw the same thing in mermaid.`,
  ...
  try {
    ...
    return {
      ...
      blocks: isTip ? [] : sanitizeTeachBlocks(parsed.blocks),
      keyPoints: isTip ? [] : (Array.isArray(parsed.keyPoints) ? parsed.keyPoints.slice(0, 3) : []),
    };
  } catch (error) {
