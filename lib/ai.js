...
const PLAN_SHAPE = {
  ...
  deep_dive: { objectives: '2-4', label: 'Deep Dive (15-20 min)', steps: '6-10' },
  project_quest: { objectives: '3-5', label: 'Project Quest (20-60 min)', steps: '7-12' },
};
...
const PLAN_MAX_STEPS = { quick_tip: 2, standard: 4, deep_dive: 10, project_quest: 12 };

const PLAN_TOKENS = { quick_tip: 2500, standard: 3500, deep_dive: 7000, project_quest: 8000 };
...
export async function generateLessonPlan(topic, learnerProfile, options = {}) {
  ...
  const tools = await orderToolsByTopic(topic, resolveTools(learnerProfile));
  const catalog = options.catalog || (await getMergedTools());
  const lineupText = formatLineupForPrompt(await getModelLineup(), tools.map((t) => t.id));
