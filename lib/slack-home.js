...
// The standing line. Streak lives in client-side progression data and isn't
// readable from the server, so the tab shows the server-true numbers (level, XP,
// rank) and sends people to the app for the full picture.
function standingSection(snapshot) {
  const text = snapshot.onBoard
    ? `*Level ${snapshot.level}*  ·  *${snapshot.totalXp.toLocaleString()} XP*  ·  *#${snapshot.rank}* of ${snapshot.totalPeople} on the leaderboard`
