...
import {
  ...
  planCapyFind, capyFindIds, capyFindCount,
...
// Collected a capybara by clicking it. Pays 5 XP the first time per egg, plus
// the one-time collection bonus and badge if this click finishes the roster.
// `roster` is FINDABLE_EGG_IDS — passed in so the rules stay a pure function.
export function onCapyFind(learnerId, eggId, roster) {
  if (!learnerId || !eggId) return null;
  return apply(learnerId, planCapyFind(readState(learnerId), eggId, { roster }));
}

// Which capybaras this learner has collected, read from their own ledger.
export function getCapyFinds(learnerId) {
  if (!learnerId) return new Set();
