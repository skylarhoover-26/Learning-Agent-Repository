...
// Remove a single notification — e.g. dismissed from the bell after viewing.
export function dismissNotification(id) {
  write(read().filter((n) => n.id !== id));
}

// Remove every notification (the bell's "Clear all"). Unfinished lessons are
// tracked separately and untouched.
...
export function recordXpNotifications(result) {
  ...
  // A collected capybara gets its own row. Without this it landed in the feed as
  // a bare "+5 XP earned", indistinguishable from any other trickle of XP — so
  // the thing the learner just went looking for left no trace that it happened,
  // and the running count was invisible.
  const capy = result.capyFind;
  if (capy) {
    addNotification({
      type: 'capybara',
      emoji: '🦫',
      title: capy.completed
        ? 'Every capybara found!'
        : `Capybara collected · ${capy.found} of ${capy.total}`,
      detail: capy.completed
        ? `+${result.xpAwarded} XP · the Capybara sidekick is unlocked`
        : `+${result.xpAwarded} XP · ${capy.total - capy.found} to go`,
      // Completing it sends them where the reward actually is.
      href: capy.completed ? '/profile' : '/achievements',
    });
  }
  // Suppressed for a capybara collect: the row above already states the XP, and
  // two rows for one click reads as a bug.
  if (result.xpAwarded > 0 && !capy) {
    addNotification({
      type: 'xp',
      emoji: '⚡',
