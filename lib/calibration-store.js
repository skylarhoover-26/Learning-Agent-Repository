...
// How much a side-weight earned on SOMEONE ELSE'S question counts toward a
// competency. Real evidence, but weaker than the question written to measure the
// thing — half credit.
const SIDE_WEIGHT = 0.5;

// Grade the quiz: for each measured competency, what the learner earned on the
// questions that measure it.
//
...
//
// The denominator is what answering PERFECTLY would earn, so a flawless run
// reads 1.00. It is deliberately NOT "the most any option could award": answers
// carry small side-weights on other competencies, and some sit on options that
...
// raise an unrelated competency.
//
// ONLY OWNING QUESTIONS SET THE DENOMINATOR. That's the fix for the results
// screen showing Communication at a flat zero. Side-weights used to be summed
// into the denominator too, so with the shipped question set Communication was
// graded out of 1.8 — 1.0 from its own question, plus 0.3 borrowed from the
// privacy question and 0.5 from the eval question. Answering the Communication
// question perfectly and the other two imperfectly capped you at 0.56, and
// getting it wrong bottomed out the bar. A competency has to be gradeable on the
// question written to measure it; credit earned elsewhere is a bonus on top,
// never a requirement, and never enough on its own to pass for mastery.
//
...
export function calculateSkills(answers, scenarios) {
  ...
  const earned = {};   // credit from the competency's OWN questions
  const bonus = {};    // credit from other questions' side-weights
  const possible = {}; // what the OWN questions' best answers were worth
  ...
  for (const scenario of scenarios || []) {
    ...
    for (const key of measured) {
      const got = chosen.scores?.[key] || 0;
      if (scenario.competency === key) {
        const onOffer = ideal.scores?.[key] || 0;
        if (onOffer <= 0) continue;
        possible[key] = (possible[key] || 0) + onOffer;
        earned[key] = (earned[key] || 0) + got;
      } else if (got > 0) {
        bonus[key] = (bonus[key] || 0) + got;
      }
    }
  }
  ...
  const skills = {};
  for (const key of measured) {
    // No denominator means the authoring is broken — a live question claims this
    // competency but its best answer awards nothing for it. Report nothing rather
    // than a zero, which the results screen would render as "Beginner".
    if (!possible[key]) continue;
    const total = (earned[key] || 0) + (bonus[key] || 0) * SIDE_WEIGHT;
    skills[key] = clamp01(total / possible[key]);
  }
  return skills;
}
