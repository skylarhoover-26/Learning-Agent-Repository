...
export async function writeDailyLessons(date, data) {
  try {
    ...
    // Overwrite in place — see the note in api/curriculum/daily. Deleting first
    // leaves the pathname 404ing briefly, which breaks a read straight after.
...
export async function getAvailableDates(maxDates = 30) {
  try {
    const { blobs } = await listJsonBlobs({ prefix: BLOB_PREFIX, limit: maxDates });
    return blobs
      .map(b => {
