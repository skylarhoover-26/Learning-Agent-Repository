...
import { mirrorActivityEvent } from '@/lib/supabase-store';
...
//
// Discovery goes through listJsonBlobs, which spans both blob stores. Listing
// only the default (public) store meant this log stopped showing anything
// written after the private-store cutover — entries were being recorded the
// whole time, into a store nothing here was looking at.
//
// Every entry is ALSO mirrored into Supabase (activity_events). Blob remains the
// source of truth and the read path below is unchanged; the mirror exists so the
// same data can be reported on — grouped by person, team, or date range —
// which per-day blob listing can't do. Entries written before that shipped exist
// only in blob.
...
// Fetch a blob's JSON fresh — entries are written once and never change, but
// the day list can grow between reads. Reads by pathname (not URL) because the
// blobs are private now; readJsonBlob handles the authenticated fetch and the
// legacy public fallback.
async function fetchJson(blob) {
  return readJsonBlob(blob.pathname, { fresh: true });
}
...
async function getDayEntries(date) {
  ...
  try {
    const { blobs } = await listJsonBlobs({ prefix: `audit/${date}/`, limit: MAX_ENTRIES_PER_DAY });
  ...
  try {
    const { blobs } = await listJsonBlobs({ prefix: `audit/${date}.json`, limit: 1 });
    if (blobs.length) {
...
export async function logAuditEntry({
  ...
  try {
    ...
    // Mirror into Supabase so this is reportable (grouped, joined, filtered by
    // date range) rather than only listable a day at a time. Deliberately NOT
    // awaited: the blob write below is the one that has to succeed, and a slow
    // Supabase round-trip must not add latency to a lesson finishing.
    mirrorActivityEvent(entry).catch(() => {});

    // Each entry is its own object keyed by a unique id — no shared file, so
    // concurrent writes can't overwrite one another.
    await writeJsonBlob(`audit/${date}/${id}.json`, entry, {
...
export async function getAuditDates() {
  try {
    ...
    const { folders = [], blobs = [] } = await listJsonBlobs({
      prefix: 'audit/',
      mode: 'folded',
      limit: 1000,
    });
