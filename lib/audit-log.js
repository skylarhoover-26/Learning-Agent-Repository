...
async function getExistingEntries(date) {
  try {
    ...
    // Read fresh — the blob is overwritten on every event, so a cached read
    // would drop entries.
...
export async function logAuditEntry({
  ...
  try {
    ...
    await put(auditKey(date), JSON.stringify(entries), {
      ...
      // Without these, the second event of the day throws (blob exists) and is
      // swallowed — which is why only the first entry was ever logged.
      allowOverwrite: true,
