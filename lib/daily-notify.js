...
// `startPath` is the app path the "Start today's pick" link points at. Prefer the
// resolved lesson URL: /daily is a pass-through that has to look the pick up in
// the browser before forwarding, which flashed a "Finding today's pick…" screen
...
function buildMessage(profile, startPath = '/daily') {
  ...
  return [
    ...
    '',
    `👉 Start today's pick: ${APP_URL}${startPath}`,
    '',
...
export async function sendDailyNotifications(trigger = 'manual') {
  ...
  for (const email of emails) {
    ...
    // Resolve the pick here so the DM can deep-link to the lesson instead of
    // bouncing through /daily. Best-effort: a failure here must never stop the
    // nudge going out, so fall back to the /daily redirect.
    let startPath = '/daily';
    try {
      ...
      console.error(`resolveDailyPick failed for ${email}, linking to /daily:`, error?.message);
    }
    const text = buildMessage(profile, startPath);
