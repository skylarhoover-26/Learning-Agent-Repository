...
import { contentDayKey, contentDayWeekday } from './content-day';
...
export async function sendDailyNotifications(trigger = 'manual') {
  ...
  // Feedback #214: competitive framing (your rank, and who is just ahead) — on
  // MONDAYS ONLY, per Skylar: at the start of the week it reads as a challenge, and
  // every morning it reads as a leaderboard nagging you.
  //
  // Read ONCE for the whole send, not per recipient. It's the same board for
  // everybody, and a per-person read would mean one leaderboard build per DM.
  const isMonday = contentDayWeekday() === 'Monday';
  const board = isMonday ? await getLeaderboard().catch(() => null) : null;
  const ranked = (board?.people || [])
    .filter((p) => p?.totalXp > 0)                 // nobody is "ranked #187 of 199" at 0 XP
    .sort((a, b) => b.totalXp - a.totalXp);

  // { rank, total, aheadName, aheadGap } for one learner, or null when they aren't
  // on the board yet — in which case Monday just looks like any other day for them,
  // which is better than telling someone they're last.
  function rankFor(learnerId) {
    if (!ranked.length || !learnerId) return null;
    const idx = ranked.findIndex((p) => p.learnerId === learnerId);
    if (idx === -1) return null;
    const ahead = idx > 0 ? ranked[idx - 1] : null;
    return {
      rank: idx + 1,
      total: ranked.length,
      // First place has nobody to chase, so the line is just the rank.
      aheadName: ahead ? firstNameOf({ display_name: ahead.name }, ahead.name) : null,
      aheadGap: ahead ? Math.max(0, ahead.totalXp - ranked[idx].totalXp) : 0,
    };
  }
  ...
  for (const email of emails) {
    ...
    const { blocks, text } = buildDailyMessage({
      ...
      slackLessonEnabled,
      aiCopy,
      // Null on any day but Monday, and buildDailyMessage re-checks the weekday, so
      // the rule can't be half-applied from one side.
      rank: rankFor(resolveLearnerId(profile)),
    });
