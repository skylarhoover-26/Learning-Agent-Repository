...
/**
 * Which build step is in flight, and which are done, at `elapsed` seconds.
 *
 * The last step never completes on its own — the finished lesson unmounting the
 * loader is what completes it. Same rule the progress bar follows: a checklist
 * showing every box ticked while you still wait reads as stuck.
 */
export function buildProgress(elapsed, format) {
  const pace = buildPace(format);
  const frac = elapsed / pace;
  let active = 0;
  BUILD_STEPS.forEach((step, i) => {
    if (frac >= step.at) active = i;
  });
  return BUILD_STEPS.map((step, i) => ({
    ...step,
    done: i < active,
    active: i === active,
  }));
