...
// Atomically append one note to a record, DB-side, via the append_feedback_note
// SQL function (see docs/supabase-schema.sql). This is a single UPDATE — it
// reads and writes the note thread in one statement, so concurrent notes (or
// notes from different admins) can never clobber each other the way a
// read-modify-write of the whole record can. Returns the updated `raw` record,
// or null when Supabase is off, the row is missing, or on error — the caller
// then falls back to the blob read-modify-write path.
export async function sbAppendFeedbackNote(id, note) {
  const supabase = getSupabase();
  if (!supabase || !id) return null;
  try {
    const { data, error } = await supabase.rpc('append_feedback_note', {
      p_id: id,
      p_note: note,
    });
    if (error) throw error;
    return data ?? null; // null = no row matched that id
  } catch (err) {
    console.error('Supabase feedback note append failed:', err?.message || err);
    return null;
  }
}
