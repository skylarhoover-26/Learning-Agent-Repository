...
export function buildDailyMessage({
  ...
  const buttons = [{
    ...
    url: `${appUrl}${startPath}`,
    action_id: 'daily_open_app',
    style: 'primary',
  }];
  if (slackLessonEnabled && pick?.topic) {
    buttons.push({
      type: 'button',
      text: { type: 'plain_text', text: 'Take it here in Slack' },
  ...
  // can't render blocks, so it says the same things in the same order.
  const text = [
    greeting,
    '',
    topic.replace(/\*/g, ''),
    reason || null,
    '',
    `Take it in the app: ${appUrl}${startPath}`,
    '',
