...
import { formatLabel, formatDuration } from './lesson-formats';
...
// are taking"). Today's Pick always opens as a Quick Lesson (PICK_FORMAT in
// lib/daily-pick-lesson.js, and FORMAT in lib/slack-lesson.js for the in-DM
// version), but both the label and the duration are derived rather than
// hardcoded so they stay right if that ever changes.
//
// This line now DOES carry a duration, reversing the earlier no-time-claims rule
// (see the note on OPENERS). That rule was about a rotating pool of invented
// promises that contradicted each other; this is one number, derived from the
// format's own shape, on the line that says what you are about to open. The
// condition it came with still holds: exactly ONE time claim in the message, so
// lib/daily-message-ai.js is forbidden from writing another.
// "*Quick Lesson:* Prompt chaining for QBR prep · about 3-5 min" — the label and
// topic keep the shape feedback #190 asked for, and the estimate trails as
// metadata rather than crowding into the bold.
function trainingLineFor(pick) {
  if (!pick?.topic) return null;
  const format = pick.format || 'standard';
  return `*${formatLabel(format)}:* ${pick.topic} \u00b7 about ${formatDuration(format)}`;
}
...
export function buildDailyMessage({
  ...
  const trainingLine = trainingLineFor(pick);
  ...
  const text = [
    ...
    topic.replace(/\*/g, ''),
    reason || null,
    trainingLine ? trainingLine.replace(/[*_]/g, '') : null,
    '',
    canStartInSlack
