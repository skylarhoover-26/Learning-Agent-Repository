...
const TRIAGE_FIELDS = ['status', 'priority', 'doneBy', 'doneAt'];
...
// Fill in a category-based default priority on any record that doesn't have one
// yet (real in-app submissions included). Never overrides an existing priority.
// Returns the full record set (newest first, with defaults applied) plus how
// many were updated, so callers can both persist and render in one pass.
export async function backfillPriorities() {
  const { blobs } = await list({ prefix: RECORD_PREFIX });
  const records = (await Promise.all(blobs.map(fetchBlobJson))).filter(Boolean);
  let updated = 0;
  const result = [];
  for (const record of records) {
    let rec = record;
    if (rec.priority == null && autoPriority(rec.category)) {
      rec = { ...rec, priority: autoPriority(rec.category) };
