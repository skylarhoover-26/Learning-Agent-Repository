...
import { putMedia } from '@/lib/blob-media';
...
export async function uploadFeedbackScreenshot(dataUrl) {
  ...
  try {
    // Goes to the private media store when configured; the admin UI reads it
    // back through /api/feedback/media rather than by URL.
    const blob = await putMedia(`${SHOT_PREFIX}shot.${parsed.ext}`, parsed.buffer, {
