...
export async function uploadFeedbackScreenshot(dataUrl) {
  ...
  try {
    const blob = await put(`${SHOT_PREFIX}shot.${parsed.ext}`, parsed.buffer, {
      // Deliberately PUBLIC, unlike the JSON records. The admin UI renders these
      // straight into <img src>, which can't carry an auth header — making them
      // private needs an authenticated proxy route first. Tracked as the
      // residual half of F-05 in docs/security/STATUS.md.
      access: 'public',
...
async function writeFeedbackBlob(record) {
  ...
  await writeJsonBlob(key, record, { cacheControlMaxAge: 0 });
}
...
// Fetch a blob's JSON fresh, or null if it can't be read. Reads by pathname —
// the records are private now, so a bare URL fetch would 401.
...
export async function listFeedbackFromBlob() {
  try {
    const { blobs } = await list({ prefix: RECORD_PREFIX });
    const records = await Promise.all(blobs.map((b) => fetchBlobJson(b)));
    return records
      .filter(Boolean)
