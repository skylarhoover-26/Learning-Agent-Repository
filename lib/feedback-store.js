...
export async function backfillPriorities() {
  ...
  for (const record of records) {
    let rec = record;
    // Backfill anything still missing a priority OR a feature area — this pass
    // also seeds the feature tag onto the existing backlog. Never overrides a
    // value that's already set (AI's call at submit time or an admin override).
    const needs = rec.category !== 'Praise' && (rec.priority == null || rec.feature == null);
    if (needs) {
      const classification = await classifyFeedback(rec);
      if (classification) {
        rec = { ...rec };
        if (rec.priority == null) {
          rec.priority = classification.priority;
          rec.aiReason = classification.reason;
          rec.aiBugVerdict = classification.bugVerdict;
          rec.priorityIsAiAssigned = true;
        }
