...
export async function backfillPriorities() {
  ...
  for (const record of records) {
    ...
    if (rec.priority == null && rec.category !== 'Praise') {
      ...
      if (classification) {
        rec = { ...rec, priority: classification.priority, aiReason: classification.reason, aiBugVerdict: classification.bugVerdict, priorityIsAiAssigned: true };
...
export async function reclassifyAiPriorities() {
  const { blobs } = await list({ prefix: RECORD_PREFIX });
  const records = (await Promise.all(blobs.map(fetchBlobJson))).filter(Boolean);
  const targets = records.filter((r) => r.category !== 'Praise' && r.priorityIsAiAssigned !== false);
