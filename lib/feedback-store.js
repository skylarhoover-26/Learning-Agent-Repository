...
export async function backfillPriorities() {
  const { blobs } = await list({ prefix: RECORD_PREFIX });
  const records = await Promise.all(blobs.map(fetchBlobJson));
  const toUpdate = records.filter(
    (r) => r && r.priority == null && autoPriority(r.category)
  );
  for (const record of toUpdate) {
    await saveFeedback({ ...record, priority: autoPriority(record.category) });
  }
