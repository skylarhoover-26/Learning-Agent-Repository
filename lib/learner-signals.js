...
export function signalSignature(profile) {
  const { tasks, goals, projects } = readSignals(profile);
  // id AND label, not `id || label`. Every tool the learner typed in themselves
  // serializes as `{ id: 'other', label: '...' }` (see serializeTools), so reading
  // the id alone made all custom tools identical to this hash: dropping
  // "Perplexity" and adding "Midjourney" left the signature unchanged, and every
  // cache keyed on it — AI News ranking, Today's Pick, the pre-generated daily
  // lesson — kept serving results built for the old toolset until the day rolled
  // over. Catalog tools were always fine; only typed-in ones went unnoticed.
  const tools = Array.isArray(profile?.preferred_tools)
    ? profile.preferred_tools.map((t) => (
      typeof t === 'string' ? t : `${t?.id || ''}:${t?.label || ''}`
    )) : [];
  const parts = [
    profile?.department || '',
    profile?.sub_team || '',
