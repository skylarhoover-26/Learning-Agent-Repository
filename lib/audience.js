...
export function playerRoleContext(profile) {
  ...
  // Projects are named, real work, which makes them the best backdrop available —
  // but they stay BACKDROP. Games personalize mainly through which topic gets
  // offered (lib/game-topics.js); the rules below still hold, so a question never
  // depends on knowing this person's project.
  const projects = activeProjects(profile).slice(0, 2).map((p) => p.title).join(', ');
  const details = [
    ...
    projects ? `- Work they have in flight: ${projects}.` : null,
  ].filter(Boolean);
