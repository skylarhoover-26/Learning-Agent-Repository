...
// is worth five minutes of their morning.
//
...
// Returns { greeting, why } or null.
//   greeting — one short line, warm, uses their first name
//   why      — 1-2 short sentences on why THIS topic is worth their time
export async function generateDailyNudge({ profile, pick, firstName }) {
  const topic = pick?.topic || pick?.title;
  if (!topic) return null;

  const system = [
    'You write the two-line opening of a friendly internal Slack message that nudges a coworker to take a 5-minute AI lesson.',
    'You are writing for Housecall Pro employees. Plain, warm, human. Never corporate.',
    '',
    'Return ONLY JSON: {"greeting": "...", "why": "..."}',
    '',
    'GREETING:',
    `- One short sentence that greets them by first name (${firstName}). It is sent on a weekday morning.`,
    '- Vary it. Do not always start the same way.',
    '- No hype, no exclamation stacking, at most one emoji and usually none.',
    '',
    'WHY:',
    `- 1-2 SHORT sentences on why "${topic}" is genuinely worth five minutes to THIS person.`,
  ...
  try {
    ...
    const greeting = clean(parsed.greeting);
    const why = clean(parsed.why);
    if (!greeting || !why) return null;
    // Length caps rather than trust: this goes out unreviewed.
    return { greeting: greeting.slice(0, 200), why: why.slice(0, 400) };
  } catch (error) {
