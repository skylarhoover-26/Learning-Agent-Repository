...
// Which competencies a given question set actually measures: the ones a live
// question is ABOUT. The results screen shows only these.
//
// This used to count any competency touched by any answer's side-scores, which
// let a competency with no question of its own earn a full score off two or
// three side-weights borrowed from other questions — someone could be rated
// Expert in Data Literacy having never been asked a data question. Side-scores
// still count toward a competency that qualifies on its own; they just can't
// conjure a measurement out of nothing.
export function measuredCompetencies(questions) {
  const seen = new Set();
  for (const q of activeQuestions(questions)) {
    if (SKILL_KEYS.includes(q.competency)) seen.add(q.competency);
  }
...
// --- Per-learner answer order ------------------------------------------------
// Everyone answers the SAME questions in the SAME order. Only the options inside
// each question move, and only per person.
//
// This exists because an authored set almost always parks the best answer in the
// same slot — questions drafted in one sitting (or with an LLM's help) drift
// toward "the third one is right", and learners notice long before we do. Moving
// the slot per person removes the tell without turning two people's quizzes into
// two different quizzes.
//
// The order comes from a seed rather than Math.random precisely so it is STABLE:
// a refresh, the back button, or an incidental re-render must never reorder the
// options someone is halfway through reading.

// FNV-1a. Small and dependency-free; mixes well enough that adjacent seeds like
// "a@x.com:privacy" and "a@x.com:prompting" don't produce related orders.
...
// mulberry32: a tiny seeded PRNG. We only need uniform-enough draws to pick a
// permutation, not cryptographic quality.
...
// Reorder ONE question's answers for one learner.
//
// Returns the question with shuffled answers, `best` repointed at wherever the
// best answer landed, and `originalIndex` mapping each display slot back to the
// authored slot.
//
// Scoring MUST go through originalIndex. calculateSkills looks answers up in the
// AUTHORED array, and saved runs store the authored index too — persisting a
// display index would score the wrong option and quietly corrupt calibration
// history against past runs.
export function shuffleAnswers(question, seed) {
  const answers = question?.answers || [];
  if (answers.length < 2) {
    return { ...question, originalIndex: answers.map((_, i) => i) };
  }

  const random = seededRandom(hashSeed(`${seed}:${question.id}`));
  ...
  return {
    ...
    best: order.indexOf(question.best),
    originalIndex: order,
  };
}

// Shuffle a whole set. `seed` identifies the learner (their email), so the order
// is stable for them and differs across people. Without one we fall back to a
// fixed seed: still varies the slot per question, just not per person.
export function shuffleQuiz(questions, seed) {
  const key = seed || 'anonymous';
  return (Array.isArray(questions) ? questions : []).map((q) => shuffleAnswers(q, key));
}
