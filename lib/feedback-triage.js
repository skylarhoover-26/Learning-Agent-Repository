...
import { FEATURE_AREAS, FEATURE_HINTS } from './feedback-features';
...
const SYSTEM_PROMPT = [
  ...
  'ALSO tag which part of the app the feedback is about, choosing exactly one feature area from this list. Use the "Filed from page" path as a strong hint when present, and the report text otherwise. If it does not clearly fit one area or spans several, use "Other".',
  FEATURE_AREAS.map((area) => `- ${area}: ${FEATURE_HINTS[area] || area}`).join('\n'),
  `Return ONLY JSON: {"priority": "<one of: ${PRIORITY_LEVELS.join(', ')}>", "reason": "<one sentence explaining the call>", "bugVerdict": "likely_bug" | "not_bug" | "unclear" | null, "feature": "<one of: ${FEATURE_AREAS.join(', ')}>"}`,
  'No markdown fences, no extra text.',
].join('\n');

// Classify one non-Praise feedback submission with a cheap model: an urgency
// priority (independent of the user-picked category), a suggested feature area,
// plus — for Bug reports only — whether it actually describes a real,
// reproducible bug. Returns { priority, reason, bugVerdict, feature } or null if
// classification fails (never blocks the submission itself; caller leaves the
// fields unset on failure).
export async function classifyFeedback(record) {
  ...
  try {
    ...
    const feature = FEATURE_AREAS.includes(parsed.feature) ? parsed.feature : null;
    return {
