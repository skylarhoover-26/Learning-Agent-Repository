...
export function useTts() {
      ...
      if (!audioCacheRef.current.has(key)) {
        try {
          ...
          if (res.ok) {
            ...
            if (blob.size >= 100) audioCacheRef.current.set(key, URL.createObjectURL(blob));
          }
        } catch {
          // ignore — speak() will fetch this line live if needed
        }
      }
      done++;
  ...
  return { isSpeaking, isPaused, isLoading, error, speak, pause, resume, stop, toggle, setRate, prime };
}
