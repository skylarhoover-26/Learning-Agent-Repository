...
const SKILL_DEFS = [
  ...
  { category: 'Foundations', name: 'Model Selection', level: 'intermediate', keywords: ['model', 'which model', 'model selection', 'model tier', 'reasoning model', 'pick a model', 'opus', 'sonnet', 'haiku', 'gpt', 'gemini', 'flash', 'pro'] },
...
// Exported so Today's Pick can verify that a reframed topic still credits the
// skill the heatmap chose (lib/daily-pick.js). A reframed title that matched a
// DIFFERENT skill would quietly update the wrong square on lesson completion.
export function matchTopicToSkill(topic) {
  ...
  for (const skill of SKILL_DEFS) {
    // A skill's own NAME is the strongest claim it has, so it counts double.
    // Without this, "Reasoning Models" still lost — Model Selection lists
    // "reasoning model" as a keyword (picking one IS model selection), which
    // outscored the Reasoning Models skill on its own name and left that square
    // permanently dark. A topic that says a skill's name outright is about that
    // skill, whatever else it mentions.
    let score = lower.includes(skill.name.toLowerCase()) ? skill.name.length * 2 : 0;
    for (const kw of skill.keywords) {
      // Score by how SPECIFIC the match is (keyword length), not how many
      // keywords hit.
      //
      // Counting hits meant a one-word generic keyword beat a precise multi-word
      // one, because equal counts fall to whichever skill SKILL_DEFS lists first.
      // Two skills could therefore never credit themselves: "System Prompts"
      // scored 1 for "system prompt" and 1 for Prompt Basics' "prompt" and lost
      // the tie; "Reasoning Models" scored 1 for "reasoning" and lost to Model
      // Selection's "model". Both squares were unreachable — every lesson on them
      // credited the other skill, so the heatmap showed a permanent gap in one
      // place and inflated mastery in another.
      //
      // Length is the right tie-breaker because a longer keyword is a narrower
      // claim: matching "system prompt" says far more about a topic than matching
      // "prompt" does.
...
// Default skill → level map, e.g. { 'RAG & Grounding': 'advanced', ... }.
// Admins can override these via the Skill Levels admin page (stored separately).
export function getSkillLevelDefaults() {
  return Object.fromEntries(SKILL_DEFS.map(d => [d.name, d.level]));
}

// Catalog used to render the admin editor: name, category, and default level.
export const SKILL_CATALOG = SKILL_DEFS.map(d => ({
  name: d.name,
  category: d.category,
  defaultLevel: d.level,
}));
