...
import { getLeaderboardTotals, getProfileMetaMap, getCapyFindCounts } from './supabase-store';
import { FINDABLE_EGG_IDS } from './easter-eggs';
...
export async function buildLeaderboard() {
  ...
  const [totals, metaMap, orgResult, capyMap] = await Promise.all([
    getLeaderboardTotals(),
    getProfileMetaMap(),
    getOrgData().catch(() => ({ data: null })),
    // Best-effort: a failed capybara read must never cost us the leaderboard.
    getCapyFindCounts().catch(() => null),
  ]);
  const totalsMap = totals || new Map();
  const meta = metaMap || new Map();
  const { data: orgData } = orgResult;
  const capyFinds = capyMap || new Map();
  // Count only eggs that are currently live, so a retired egg can't leave someone
  // showing 13 of 12.
  const capyCountFor = (email) => {
    const found = capyFinds.get(email);
    if (!found) return 0;
    return FINDABLE_EGG_IDS.filter((id) => found.has(id)).length;
  };
  ...
  for (const dept of orgData?.departments || []) {
    for (const e of dept.employees || []) {
      ...
      const person = {
        ...
        capyFinds: capyCountFor(emailLower),
      };
      people.push(person);
...
export async function getStoredLeaderboard() {
  return readJsonBlob(LEADERBOARD_CACHE_KEY, { fresh: true });
}
...
export async function buildAndStoreLeaderboard() {
  ...
  try {
    await writeJsonBlob(LEADERBOARD_CACHE_KEY, snapshot, { cacheControlMaxAge: 0 });
  } catch (e) {
