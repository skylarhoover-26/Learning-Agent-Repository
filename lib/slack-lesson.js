...
// Interactive teach blocks (flashcards, reveals, tabs, diagrams) have no Slack
// equivalent, so their CONTENT is flattened into text rather than dropped — the
// teaching in them is often the best part of the step.
function teachBlockText(block) {
  if (block?.type === 'flashcards') {
    return [block.title ? `*${block.title}*` : null,
      ...(block.cards || []).map((c) => `• *${c.front}* — ${c.back}`)].filter(Boolean).join('\n');
  }
  if (block?.type === 'reveal') {
    return [`*${block.title || 'Example'}*`, toMrkdwn(block.content)].filter(Boolean).join('\n');
  }
  if (block?.type === 'tabs') {
    return [block.title ? `*${block.title}*` : null,
      ...(block.tabs || []).map((t) => `*${t.label}*\n${toMrkdwn(t.content)}`)].filter(Boolean).join('\n\n');
  }
  if (block?.type === 'diagram') {
    return [block.title ? `*${block.title}*` : null,
      ...(block.steps || []).map((s, i) => `${i + 1}. *${s.label}* — ${s.detail}`)].filter(Boolean).join('\n');
  }
  return null;
}
...
async function postStep(email, session, profile) {
  ...
  await postSlackMessage(session.channel, text, blocks);
...
export async function startSlackLesson({ email, channel, topic }) {
  ...
  let teach = cached?.teach || {};

  if (!plan) {
    await postSlackMessage(channel, 'Building your lesson. This takes up to a minute.', [
  ...
  if (!plan?.steps?.length) {
    await postSlackMessage(channel, "I couldn't build that lesson.", [{
      type: 'section',
  ...
  await postSlackMessage(channel, `Starting: ${topic}`, intro);
  await logAuditEntry({
    type: 'lesson_start',
    endpoint: '/api/slack',
...
export async function openSlackActivity({ email, stepId, triggerId, channel = null }) {
  ...
  if (!blocks) {
    ...
    await writeSession(email, session);
    await postSlackMessage(session.channel, 'Skipping that one.', [{
      type: 'section',
  ...
  if (!opened.ok) {
    ...
    await postSlackMessage(session.channel, "That didn't open.", [
      {
        type: 'section',
        text: {
          type: 'mrkdwn',
...
export async function postCurrentStep({ email, session, channel = null }) {
  ...
  const profile = await loadProfile(email);
  await postSlackMessage(session.channel, 'Picking up where you left off.', [{
    type: 'section',
...
export async function submitSlackActivity({ email, view }) {
  ...
  await postSlackMessage(session.channel, passed ? 'Correct.' : 'Not quite.', [
...
export async function quitSlackLesson({ email }) {
  ...
  await postSlackMessage(session.channel, 'Stopped for now.', [{
    type: 'section',
...
export async function finishSlackLesson({ email }) {
  ...
  await postSlackMessage(session.channel, `Lesson complete: ${session.topic}`, [
