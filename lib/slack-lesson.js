...
async function loadProfile(email) {
  ...
  // Projects are a separate user-data key, so attach them here — this is the one
  // choke point both the plan and the teach-step calls below read from, and the
  // generators weight tasks, goals AND projects (lib/learner-signals.js). Slack
  // lessons must stay at parity with the app's.
  const work_projects = await getWorkProjects(email);
  return { ...(stored?.data || stored || {}), email, work_projects };
}
...
export async function startSlackLesson({ email, channel, topic }) {
  ...
  const intro = [
    ...
    ...((plan.objectives || []).length
      ? [{ type: 'section', text: { type: 'mrkdwn', text: `*By the end, you'll be able to:*\n${plan.objectives.map((o) => `• ${o.text}`).join('\n')}` } }]
      : []),
