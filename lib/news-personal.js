...
// How old an item may be and still count as news.
//
// FIVE days, widened from two on 2026-08-17 after measuring what the feed
// actually produces. Of 200 stored findings, 106 were arXiv papers; only 28 fell
// inside 48 hours, and exactly ONE of those was in a teachable category. The page
// was showing a single item, and 72 hours would not have changed that number at
// all — the counts only move at five days (8) and a week (12).
//
// So the two-day rule was not filtering noise, it was assuming a daily cadence
// these sources do not have. They publish papers daily and practical, teachable
// changes a couple of times a week. Tuesday's model change is still worth knowing
// on Thursday.
//
// Widened rather than loosening APPROVED_CATEGORIES on purpose: the categories
// carry the "is this teachable for an ordinary employee" judgement, which is the
// page's entire value. Trading that for volume buys commentary nobody asked for.
// The real long-term fix is more product-blog and changelog sources, which lets
// this tighten again.
//
// The original two-day value existed because a VentureBeat article from JANUARY
// was showing on the page in August. That cause is separately fixed and stays
// fixed: VentureBeat's live feed still serves that item among its newest ten, and
// the findings blob evicted by POSITION rather than date, so it never aged out.
// The scan drops old items on the way in (api/curriculum/daily) and the page
// refuses to show them on the way out. Five days is still nowhere near January.
export const MAX_AGE_HOURS = 120;

// The window in the words a reader would use. "the last 120 hours" is technically
// what the constant says and not how anyone thinks about time.
export const MAX_AGE_LABEL = MAX_AGE_HOURS % 24 === 0
  ? `${MAX_AGE_HOURS / 24} days`
  : `${MAX_AGE_HOURS} hours`;

/**
 * Is this item recent enough to be news?
 *
 * An item with no parseable date fails. That is deliberate: the whole promise
 * here is recency, and "we cannot tell how old this is" is not a reason to
 * assume it is fresh. Across all ten feeds, 160 items sampled, every single one
 * carried a valid date — so this rejects almost nothing in practice, and what it
 * does reject is exactly what we cannot vouch for.
 */
export function isFresh(item, nowMs = Date.now()) {
  const ms = new Date(item?.publishedAt || 0).getTime();
  if (!Number.isFinite(ms) || ms <= 0) return false;
  return (nowMs - ms) <= MAX_AGE_HOURS * 3600000;
}
 ...
 *
 *   0. Freshness. Answers "is this news at all". A correct, well-matched item
 *      from January is not news in August, however well it scores.
 ...
 */
export function isDefaultVisible(item, nowMs = Date.now()) {
  if (!isFresh(item, nowMs)) return false;
  if (!isApproved(item)) return false;
  return typeof item?.score !== 'number' || item.score >= NOISE_FLOOR;
}

// splitByVisibility used to live here, returning the shown and held-back lists
// together so a "Show everything" toggle could name how many it would add. The
// toggle is gone: telling a learner about the 190 items we had just judged not
// worth their time was an invitation to go and read the noise. Callers filter
// with isDefaultVisible and there is no second list to keep in step.
...
/**
 * The exact set of items sent for personal ranking, and the order they go in.
 *
 * Shared by /ai-news and the home page's news card, and it has to be BYTE-FOR-BYTE
 * the same call from both. api/ai-news/why caches per learner per content-day
 * keyed on a signature of the item set, so two surfaces asking for two slightly
 * different lists both miss the cache, pay for their own model run, and — because
 * batch composition changes with the set — can come back with different scores
 * for the same article. Which is how the home card ended up advertising items the
 * page itself would not show.
 *
 * The score gate (isDefaultVisible) deliberately runs AFTER this: it needs the
 * score this call produces.
 */
export function rankableItems(items, nowMs = Date.now()) {
  return (items || [])
    .filter((i) => passesNewsGate(i, nowMs))
    .slice()
...
/**
 * Everything the practical feed requires before an item can be shown or ranked.
 *
 * isDefaultVisible is NOT enough on its own: it checks freshness, category and
 * score, but not identity or source. /ai-news files research separately and drops
 * untitled rows in its ranking filter, so a caller relying on isDefaultVisible
 * alone shows arXiv papers and blank cards. That is exactly what the home card
 * did.
 */
function passesNewsGate(item, nowMs = Date.now()) {
  return Boolean(
    item?.externalId && item?.title && !isResearchSource(item.sourceName)
    && isApproved(item) && isFresh(item, nowMs)
  );
}

/**
 * The best `count` items for this learner — what the home card shows.
 *
 * Same gates and same ordering as the page's default practical view, so the card
 * is a true top-N of that page rather than a second opinion. With no scores it
 * degrades to newest-first with every gate still applied: less personal than the
 * page, never contradicting it.
 */
export function topPersonalItems(items, personal, count = 3, nowMs = Date.now()) {
  const gated = (items || []).filter((i) => passesNewsGate(i, nowMs));
  return attachPersonal(gated, personal || {})
    .filter((i) => isDefaultVisible(i, nowMs))
    .sort(byBestMatch)
    .slice(0, count);
}
