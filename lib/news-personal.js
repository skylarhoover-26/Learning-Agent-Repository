...
//
// Two days, not one, because the scan runs at 1:00 AM PT: an article published
// yesterday evening is exactly what that morning's scan exists to surface, and a
// strict midnight cutoff would throw it away hours after it appeared.
//
// This exists because a VentureBeat article from JANUARY was showing on the page
// in August. The cause was not stale storage, or not only that — VentureBeat's
// live feed still serves that item among its newest ten, so every scan ingested
// it afresh, and the findings blob evicted by POSITION rather than date, so it
// never aged out. Both halves are fixed: the scan drops old items on the way in
// (api/curriculum/daily) and the page refuses to show them on the way out.
