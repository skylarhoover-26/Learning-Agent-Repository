...
export function replaceAllData(learnerId, data) {
  write('xp', learnerId, Array.isArray(data?.xpEvents) ? data.xpEvents : []);
  write('badges', learnerId, Array.isArray(data?.badgesEarned) ? data.badgesEarned : []);
  write('lessons', learnerId, Array.isArray(data?.lessonHistory) ? data.lessonHistory : []);
}
