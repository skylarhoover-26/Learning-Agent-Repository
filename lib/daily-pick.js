...
function reframeSkill(skillName, profile, seed) {
  ...
  // Two skill NAMES already fail to credit themselves — "System Prompts" scores as
  // Prompt Basics and "Reasoning Models" as Model Selection, because the matcher
  // counts keyword hits and a tie goes to whichever skill is defined first. That's
  // a pre-existing matcher bug, so demanding an exact match here would only
  // withhold personalization from those two skills without fixing anything.
