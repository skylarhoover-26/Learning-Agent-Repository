...
// Save the richer per-competency detail from AI synthesis:
//   detail = { personal: { self, measured, why }, team: {...}, org, development }
// We also derive the flat { dimension: measured } `scores` map so existing
// consumers (manager dashboard, getOverallLevel) keep working, and append a
// dated snapshot to history for trends / "see old scores".
export function saveImpactDetail(detail) {
  const existing = getScoringData() || { history: [] };
  const history = existing.history || [];
  const scores = {};
  for (const k of ['personal', 'team', 'org', 'development']) {
    scores[k] = detail?.[k]?.measured ?? null;
  }
  history.push({ scores, detail, scored_at: new Date().toISOString() });
  return saveScoringData({ scores, detail, history });
}

export function getImpactDetail() {
  return getScoringData()?.detail || null;
}
...
// 4 weeks, i.e. roughly monthly — deliberately lined up with the monthly CSC
// cycle so a manager's check-in has fresh competency scores behind it.
export const IMPACT_ASSESSMENT_INTERVAL_WEEKS = 4;
...
// Due if never assessed, or the last assessment was >= the interval ago.
