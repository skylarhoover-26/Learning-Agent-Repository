...
import { getAuthenticatedProfile } from './auth-helpers';
import { displayNameFromProfile } from './display-name';
import { getLevel as curveGetLevel, getLevelProgress as curveGetLevelProgress } from './level-curve';
...
export async function getCurrentLearner() {
  try {
    ...
    return {
      ...
      goal: profile.goal || '',
      goals: Array.isArray(profile.goals) ? profile.goals : (profile.goal ? [profile.goal] : []),
      total_sessions: 0,
...
export function getLevel(totalXp) {
  return curveGetLevel(totalXp);
}

export function getLevelProgress(totalXp) {
  return curveGetLevelProgress(totalXp);
}
