...
// Workflow status for a piece of feedback — how far along the work is, set by
// an admin from the dropdown at the bottom of each card. Independent of the
// coarse open/done/skipped disposition: a card can be "In Progress" while it
// still lives in the Pending queue.
//
// 'On Watch' is the last one on purpose: it means the work is believed done but
// is being watched before anyone calls it finished. That is a state BEFORE Done
// (the disposition), not a kind of Done — an item sitting here is still open and
// still counted as active.
export const WORK_STATUSES = ['Not Started', 'In Progress', 'Needs Review', 'Blocked', 'On Watch'];
