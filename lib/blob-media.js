...
// separate private store that PRIVATE_READ_WRITE_TOKEN points at. Reads then go
// through /api/feedback/media, which is session- and admin-gated.
//
// THE GATE: with PRIVATE_READ_WRITE_TOKEN unset this behaves exactly as before —
...
// Blob doesn't always round-trip a content type (copies can come back as
// application/octet-stream), and <video> refuses to play an octet-stream. Infer
// from the extension so playback works regardless of what storage reports.
const MIME_BY_EXT = {
  mov: 'video/quicktime',
  mp4: 'video/mp4',
  webm: 'video/webm',
...
export function mimeForPathname(pathname) {
