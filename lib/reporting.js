...
export async function buildAndStoreReport() {
  ...
  if (report.orgLoaded) {
    try {
      await put(REPORT_BLOB_KEY, JSON.stringify(report), {
        // REQUIRED by @vercel/blob v2 — see the note in lib/daily-lessons.js.
        // Without it the cache write threw and was swallowed, so the daily
        // reporting refresh never actually cached anything.
        access: 'public',
...
export async function getStoredReport() {
  try {
    const { blobs } = await list({ prefix: REPORT_BLOB_KEY, limit: 1 });
    const blob = blobs.find((b) => b.pathname === REPORT_BLOB_KEY) || blobs[0];
    if (!blob) return null;
