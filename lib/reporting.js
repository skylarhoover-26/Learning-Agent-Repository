...
export async function buildAuditRollup(days = 30) {
  ...
  const engagement = await Promise.all(
    dates.map(async (date) => {
      ...
      return { date, activeUsers: users.size, lessons, events: entries.length, activeEmails: [...users], lessonsByEmail };
    })
  );
  return { engagement, lastSeen, byType };
}

// Friendly labels for the "what people are doing" breakdown. Intermediate/outcome
// events (page views, mid-lesson steps, XP/badge grants) are dropped so the chart
...
const ACTIVITY_SKIP = new Set(['page_visit', 'lesson_continue', 'xp_earned', 'badge_earned', 'discuss', 'tones']);

function activityFromByType(byType) {
...
export async function buildReport() {
  ...
  const engagementEnriched = engagement.map((d) => {
    ...
    return { date: d.date, activeUsers: d.activeUsers, lessons: d.lessons, events: d.events, active, lessonsByEmail: d.lessonsByEmail || {} };
  });
