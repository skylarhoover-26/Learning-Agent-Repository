...
import { getOrgData, readCachedData } from './manager-data';
...
export async function buildReport() {
  ...
  // Prefer the already-cached org roster (fast) so the report build can't be held
  // up by the Snowflake/n8n webhook (which can take ~30s when stale and could push
  // the build past the function timeout before it saves the cache). Only when no
  // cached org exists at all do we fall back to a live fetch. The org cache is kept
  // fresh independently by the manager dashboard + the daily report cron.
  let orgData = await readCachedData().catch(() => null);
  if (!orgData) {
    ({ data: orgData } = await getOrgData().catch(() => ({ data: null })));
  }
