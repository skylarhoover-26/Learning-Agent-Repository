  ...
  if (org) {
    for (const dept of org.departments || []) {
      for (const e of dept.employees || []) {
        const info = { department: dept.name, subTeam: e.subTeam || '', title: e.title || '' };
...
// Daily engagement from the audit log: distinct active users + lessons completed
// + total events per day. Global (app-wide), not sliced by team — that's a
// later refinement. Read-only over the per-day audit blobs.
export async function buildEngagement(days = 14) {
  ...
  return Promise.all(
    dates.map(async (date) => {
      ...
      for (const e of entries) {
        ...
        if (email && email !== 'unknown' && !EXCLUDED.has(email)) users.add(email);
...
export async function buildReport() {
  ...
  await Promise.all(
    [...byUser.entries()].map(async ([learnerId, urls]) => {
      ...
      const orgInfo = org.byEmail.get(String(learnerId).toLowerCase()) || org.byName.get(norm(name)) || {};
      const lastActive = [lastXpAt, lastLessonAt, p.updated_at].filter(Boolean).sort().at(-1) || '';

      people.push({
        learnerId,
        name,
        email: typeof learnerId === 'string' && learnerId.includes('@') ? learnerId : (p.email || ''),
        department: p.department || orgInfo.department || 'Unassigned',
        subTeam: orgInfo.subTeam || p.sub_team || '',
        manager: org.managerOfName.get(norm(name)) || '',
        title: orgInfo.title || '',
        totalXp,
  ...
  const engagement = await buildEngagement(14);
