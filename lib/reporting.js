...
export async function buildAuditRollup(days = 14) {
  ...
  const engagement = await Promise.all(
    dates.map(async (date) => {
      ...
      return { date, activeUsers: users.size, lessons, events: entries.length, activeEmails: [...users] };
    })
  );
...
export async function buildReport() {
  ...
  const { engagement, lastSeen, byType } = await buildAuditRollup(14);
  ...
  const engagementEnriched = engagement.map((d) => {
    const active = (d.activeEmails || []).map((em) => {
      ...
      return p ? { name: p.name, department: p.department } : { name: nameFromId(em), department: 'Other' };
    }).sort((a, b) => a.name.localeCompare(b.name));
    return { date: d.date, activeUsers: d.activeUsers, lessons: d.lessons, events: d.events, active };
  });
