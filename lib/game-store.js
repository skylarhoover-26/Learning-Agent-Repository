...
export function saveGameResult(slug, result) {
  ...
  // Every finished game pays out, however the round was built. Custom rounds used
  // to be practice-only, which quietly meant Family Feud, Jeopardy, Millionaire,
  // Two Truths and Wheel of Fortune could NEVER award XP — a generated round is
  // the only way to play them. Two rounds that look identical to the player now
  // score identically; the once-per-game-per-content-day gate in onGameComplete
  // is what stops farming, exactly as it does for the standard games.
  //
