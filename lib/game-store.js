...
import { trackGameComplete } from './track';
import { recordActivity } from './adaptive-store';
...
// The DECLARED level from onboarding, read the same way — adaptive needs it as
// the promotion ceiling and as the seed before there's any history.
function cachedDeclaredTier() {
  try {
    const raw = localStorage.getItem('learner_profile');
    return raw ? (JSON.parse(raw)?.tier || null) : null;
  } catch {
    return null;
  }
}
...
export function saveGameResult(slug, result) {
  ...
  const fraction = resultFraction(result);
  ...
  if (learnerId) {
    ...
    // A perfect run flags the XP event so the toast can show the sunglasses
    // capybara. Flagged here, on the one path every game already funnels
    // through, rather than in twelve separate results screens — same reasoning
    // as the activity logging below. If the perfect run also levels them up the
    // LevelUpModal takes over and this is skipped, which is correct: one
    // celebration per action.
    // See lib/easter-eggs.js: game-perfect-score.
    emitXp({ ...xpResult, perfectGame: fraction >= 1 });
    // Games were absent from the activity log entirely — a whole activity type
    // missing from it, and from anything reporting off it. Logged here rather
    // than in each game page so every game is covered by construction.
    trackGameComplete(slug, { fraction, score: result?.score, xp: xpResult?.xpAwarded });

    // Games feed adaptive difficulty at half a lesson's weight, and can never
    // trigger the repeated-fail demotion on their own — a bad round is weaker
    // evidence than failing a lesson twice. Best-effort: a game must still pay
    // out even if levelling throws.
    try {
      const declared = cachedDeclaredTier();
      if (declared) recordActivity(fraction * 100, { tier: declared, source: 'game' });
    } catch {
