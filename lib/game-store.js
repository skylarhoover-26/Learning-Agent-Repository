...
export function saveGameResult(slug, result) {
  ...
  // Custom rounds are practice only: record the play locally (stats/history)
  // but award no XP, so nothing feeds the leaderboard — matching the on-screen
  // copy. Only standard rounds pay out.
  if (result.custom) {
    return null;
  }
