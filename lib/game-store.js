...
export function saveGameResult(slug, result) {
  ...
  // Award capped game XP once per finished game. Wired here so all games earn
  // XP through a single path. Total games played (across all games) drives the
  // game badges.
  ...
  if (learnerId) {
    ...
    const xpResult = onGameComplete(learnerId, slug, totalGames);
    emitXp(xpResult);
    return xpResult;
  }
  return null;
