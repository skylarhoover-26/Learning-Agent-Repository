...
//               item is not fully visible for this viewer (hidden OR "coming soon"),
//               since home renders nothing in either case and the spotlight would
//               otherwise land on empty space
//
...
export const GUIDED_TOUR_STEPS = [
  ...
  // `describeProfileItems` makes the description list the items this viewer can
  // ACTUALLY see. It used to name them in a hardcoded sentence, which went wrong
  // the moment an admin hid one: the tour kept promising "My Calibration" to
  // people whose menu no longer had it. Anything the tour names has to be
  // something the person can point at.
  { element: '[data-tour="name-menu"]', popover: { title: 'Everything about you lives here', description: 'Your name up top opens your personal settings.' }, describeProfileItems: true, profileMenu: 'open' },
  ...
  // Feedback — the last thing they see, because the tour is exactly when someone
  // has just formed an opinion and has nowhere to put it. `sidebar: true` opens
  // the drawer for this step on mobile as well as desktop, since the anchor lives
  // inside the menu rather than on the page.
  { element: '[data-tour="nav-feedback"]', sidebar: true, popover: { title: 'Tell us what you think', description: 'Found a bug, or something that could be better? Send feedback from here any time — you can attach a screenshot or a screen recording, and it comes straight to the team.' } },
];
