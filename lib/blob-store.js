...
export async function deleteUserData(learnerId, dataType) {
  try {
    // Both stores — deleting only the public copy left the private one to be
    // served on the next read, so a "reset" could appear to do nothing.
    await delJsonBlob(userKey(learnerId, dataType));
    return true;
  } catch (error) {
