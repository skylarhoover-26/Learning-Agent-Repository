...
import { getLessonById } from '@/lib/structured-lessons-data';
...
const TONE_COLORS = {
  warm: 'bg-rose-50 border-rose-200 text-rose-700 dark:bg-rose-950 dark:border-rose-800 dark:text-rose-300',
  concise: 'bg-brand-50 border-brand-200 text-brand-700 dark:bg-brand-950 dark:border-brand-800 dark:text-brand-300',
...
export default function StructuredLessonPlayer({ params }) {
  ...
  useEffect(() => {
    if (stepIdx >= 2 && !tones && !loadingTones && lesson) {
      ...
      fetch('/api/lesson/tones', {
        ...
        body: JSON.stringify({
          ...
          toneContext: lesson.toneContext,
        }),
      })
        .then(r => r.json())
  ...
  }, [stepIdx, tones, loadingTones, lesson]);
    ...
    return (
      <div className="min-h-screen bg-bg-warm dark:bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/structured-lesson" className="text-brand hover:underline text-sm font-semibold">
            Back to lessons
          </Link>
        </div>
      </div>
...
function ReadStep({ lesson, onNext }) {
  const isMultiline = lesson.sourceText.includes('\n');
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-card p-8">
        ...
        <div className="bg-brand-50 dark:bg-brand-950 border border-brand-100 dark:border-brand-800 rounded-xl p-4">
          <p className="text-sm font-semibold text-brand-800 dark:text-brand-200 mb-1">Tips for a strong response</p>
...
function TryStep({ lesson, submission, setSubmission, grade, grading, onSubmit, onNext, onBack }) {
  const isMultiline = lesson.sourceText.includes('\n');
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-card p-8">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 mb-4 text-xs text-slate-600 dark:text-slate-400 font-mono">
          {isMultiline ? (
            <pre className="whitespace-pre-wrap">{lesson.sourceText}</pre>
          ) : (
            lesson.sourceText
          )}
        </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="bg-green-50 dark:bg-green-900/30 border border-green-100 dark:border-green-800 rounded-xl p-4">
                <p className="text-xs font-bold text-green-700 dark:text-green-300 uppercase tracking-wide mb-1">Strength</p>
              ...
              <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-100 dark:border-amber-800 rounded-xl p-4">
                <p className="text-xs font-bold text-amber-700 dark:text-amber-300 uppercase tracking-wide mb-1">To improve</p>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-card p-8">
        ...
        <div className="mb-6">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{userMessage}</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-card p-8">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
          Pick the one that fits best. There's no wrong answer — the goal is to think about why.
        </p>
            ...
            <button
              ...
              className={`w-full text-left p-4 rounded-xl border transition-all ${
                choice === opt.id
                  ...
                  : 'bg-white dark:bg-slate-800 text-ink dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:border-brand-200 hover:bg-brand-50 dark:hover:bg-brand-950'
              }`}
            >
              ...
              <p className="text-sm leading-relaxed whitespace-pre-wrap">{opt.message}</p>
            </button>
          ))}
        </div>
      </div>
...
function ReflectStep({ lesson, reflection, setReflection, grade }) {
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-card p-8">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
          One sentence is enough. What will you do differently next time?
        </p>
        ...
        <div className="bg-gradient-to-br from-brand-50 to-purple-50 dark:from-brand-950 dark:to-purple-950 border border-brand-100 dark:border-brand-800 rounded-xl p-6">
      ...
      <div className="flex justify-center gap-3">
        <Link
          href="/structured-lesson"
          className="inline-flex items-center gap-2 px-6 py-3 rounded-pill border border-slate-200 dark:border-slate-700 text-ink dark:text-slate-200 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
        >
