...
import {
  ...
  SCORE_LABELS, DIMENSION_LABELS,
} from '@/lib/scoring-store';
import {
  RefreshCw, ChevronRight, ChevronLeft, Loader2,
  User, Users, Building2, Brain, ArrowRight, TrendingUp, ArrowDown, Minus,
} from 'lucide-react';
...
// Check-in section of the "My Growth" page. Needs an AI Impact baseline to
// compare against; if there isn't one, it nudges the learner to the AI Impact
// tab via onGoToImpact.
export default function CheckinPanel({ onGoToImpact }) {
  ...
  useEffect(() => {
    setPreviousScores(getScores() || null);
    setLoaded(true);
  }, []);

  const visibleQuestions = CHECKIN_QUESTIONS.filter(q => {
    if (q.conditional && q.dimension === 'blocker') return noImprovement;
    return true;
  });
  ...
  function handleOptionSelect(option) {
    ...
    if (dimension === 'personal' && option.delta === 0) setNoImprovement(true);
    if (dimension === 'team' && option.delta === 0 && answers.personal?.delta === 0) setNoImprovement(true);
    advanceStep();
  }
  ...
  async function handleFollowUpSubmit() {
    ...
    try {
      ...
      setAnswers(prev => ({ ...prev, [currentQuestion.dimension]: { value: 'D', delta: 1 } }));
      setShowFollowUp(false);
      setFollowUpText('');
      advanceStep();
    } finally {
  ...
  // No baseline yet → send them to the AI Impact assessment first.
  if (!previousScores) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-card p-10 text-center">
        ...
        <h2 className="text-xl font-bold text-ink dark:text-slate-200 mb-2">No baseline yet</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 max-w-md mx-auto">
          Take the AI Impact assessment first so the check-in has a starting point to compare against.
        </p>
        <button
        ...
        >
          Go to AI Impact <ArrowRight className="w-4 h-4" />
        </button>
      </div>
    );
  }
  ...
  if (completed && newScores) {
    return <CheckInResults previous={previousScores} current={newScores} />;
  }
  ...
  return (
    <div>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">A quick progress update — takes ~2 minutes.</p>
      <div className="mb-6">
      ...
      <div className="p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {anyImproved ? 'Keep building on the momentum.' : 'Focus on completing the next module in your learning path.'}
          </p>
        </div>
              ...
              <div key={dim} className="flex items-center gap-4 p-4 rounded-xl bg-bg-warm dark:bg-slate-900 border border-slate-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {delta !== 0 && (
                      <span className={`inline-flex items-center gap-1 text-xs font-bold ${delta > 0 ? 'text-green-600' : 'text-red-500'}`}>
                        <DeltaIcon className="w-3 h-3" />{delta > 0 ? '+' : ''}{delta}
                      </span>
                    )}
                  </div>
                </div>
