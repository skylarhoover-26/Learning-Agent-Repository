...
export default function PlanLessonPlayer({ topic: topicProp, format = 'standard', onExit }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Progress + step count — plain bar with a clear "Step X of Y". */}
      <div className="flex items-center gap-3">
      ...
      {/* Persistent lesson header card — eyebrow + descriptive title + the
          "By the end, you'll be able to:" objectives, pinned on every step so
          the learner always sees the goal (restored pre-cinematic design). */}
      <div className="rounded-2xl border border-brand-200 dark:border-slate-700 bg-brand-50/50 dark:bg-slate-800 p-5">
        <p className="text-[11px] font-bold uppercase tracking-[.14em] text-brand mb-1.5">{FORMAT_LABEL[format] || 'Lesson'}</p>
        <h1 className="font-display font-bold text-ink dark:text-slate-100 leading-snug text-xl sm:text-2xl">{headline}</h1>
        {objectives.length > 0 && (
          <div className="mt-4">
      ...
      {step?.kind !== 'recap' && (
        <div className="flex items-center justify-between gap-3">
          {/* Back is a pill the same size and weight as Next, rather than the bare
              text link it was — at the bottom of a lesson the two belong to the same
              control, and a link next to a button reads as an afterthought.
              Outlined rather than filled, matching onboarding's StepNav: two solid
              buttons side by side compete for "the thing to press next". */}
          <button onClick={back} disabled={stepIdx === 0 && (!isTeachStep || safePanelIdx === 0)}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-pill border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 font-semibold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
